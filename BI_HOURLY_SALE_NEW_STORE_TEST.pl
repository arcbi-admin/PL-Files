...
$test_update = qq{ SELECT CUR_TIME, UPDATE_TIME, CASE WHEN CUR_TIME<> UPDATE_TIME THEN 0 ELSE 1 END AS STATUS 
					...
					(SELECT CASE WHEN NEW_TIME IS NULL THEN '0' ELSE TO_CHAR(NEW_TIME) END AS UPDATE_TIME 
					  FROM ( SELECT MAX(TO_NUMBER(TS_RTN_HR)) NEW_TIME
										FROM MG_HOURLY_SALES WHERE DC_DY_BSN = (TO_CHAR(SYSDATE, 'YYYY-MM-DD')) AND TO_NUMBER(ID_STR_RT) = '3010'))B };
...
if ($test eq 1){				
	$test_query = qq{ SELECT CASE WHEN NEW_TIME > 12 THEN  TO_CHAR(NEW_TIME -12) || ':00 PM' ELSE TO_CHAR(NEW_TIME) || ':00 AM' END AS UPDATE_TIME FROM (
						...
						FROM MG_HOURLY_SALES WHERE DC_DY_BSN = (TO_CHAR(SYSDATE, 'YYYY-MM-DD')) AND TO_NUMBER(ID_STR_RT) = '3010') };
...
FROM (
	...
	FROM
		...
		WHERE DC_DY_BSN = (TO_CHAR(SYSDATE, 'YYYY-MM-DD')) AND D.DIVISION NOT IN ('7500', '4000', '8000', '9000', '8500') AND TO_NUMBER(H.ID_STR_RT) = '3010' AND H.TS_RTN_HR <= (SELECT TO_CHAR(SYSDATE, 'HH24') NEW_TIME FROM DUAL)
	...
	UNION ALL
		...
		WHERE TO_DATE(Q.B_DATE,'MM/DD/YY') = TO_CHAR(SYSDATE, 'DD-MON-YY') AND Q.TYPE = '3010' AND Q.DIVISION NOT IN ('7500', '4000', '8000', '9000', '8500') 
		GROUP BY BI.MERCH_GROUP_CODE, BI.MERCH_GROUP_DESC, D.DIVISION, D.DIV_NAME)
...
<b><font size="3">3010 - Metro Banilad</font></b><br>
