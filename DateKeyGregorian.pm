...
$query2 = "SELECT * FROM (SELECT DATE_KEY AS DATE_KEY2,DATE_FLD AS DATE_FLD2 FROM DIM_DATE WHERE DATE_KEY = (SELECT DATE_KEY FROM DIM_DATE WHERE DATE_FLD = (SELECT MAX(ETL_TO_DATE) FROM INC_ETL_HISTORY)) - ((SELECT SUBSTR(K,1,2) FROM(SELECT MAX(ETL_TO_DATE) AS K FROM INC_ETL_HISTORY))-1))A, (SELECT DATE_KEY AS DATE_KEY2_LY, DATE_FLD AS DATE_FLD2_LY FROM DIM_DATE WHERE DATE_FLD = (SELECT (SUBSTR(K,1,6) || - $year_ly) FROM(SELECT DATE_FLD AS K FROM DIM_DATE WHERE DATE_KEY = (SELECT DATE_KEY FROM DIM_DATE WHERE DATE_FLD = (SELECT MAX(ETL_TO_DATE) FROM INC_ETL_HISTORY)) - ((SELECT SUBSTR(K,1,2) FROM(SELECT MAX(ETL_TO_DATE) AS K FROM INC_ETL_HISTORY))-1))))B";
