...
if ($test eq 1){
	...
	$workbook = Excel::Writer::XLSX->new("Daily Sales Performance - Concession (as of $as_of).xlsx");
	# $workbook = Excel::Writer::XLSX->new("Daily Sales Performance - Concession (October November 2013).xlsx");
...
my $csv = Text::CSV_XS->new ({ binary => 1, eol => $/ });
 open my $fh, ">", "bi_sales_perf_con.csv" or die "bi_sales_perf_con.csv: $!";
...
FROM (
	SELECT TBL.STORE_KEY, TBL.STORE_CODE, TBL.DS_KEY, TY.SALE_NET_VAL, TY.SALE_TOT_TAX_VAL, TY.SALE_TOT_DISC_VAL, LY.SALE_NET_VAL_LY, LY.SALE_TOT_TAX_VAL_LY, LY.SALE_TOT_DISC_VAL_LY FROM
		...
		FROM AGG_DLY_STR_PROD AGG 
			JOIN DIM_PRODUCT M ON AGG.PRODUCT_KEY = M.PRODUCT_KEY AND PRODUCT_CATEGORY = 2
...
my $sth = $dbh->prepare ($test);
 ...
 close $fh or die "bi_sales_perf_con.csv: $!";
...
$attachment_file_1 = "Daily Sales Performance - Concession (as of $as_of).xlsx";
