...
if ($test eq 1){
	...
	SELECT WTD_ST_DT, WTD_ST_DT_LY, WTD_EN_DT, WTD_EN_DT_LY FROM
		(SELECT TO_CHAR(DATE_FLD, 'DD Mon YYYY') AS WTD_ST_DT, TO_CHAR(DATE_FLD_LY, 'DD Mon YYYY') AS WTD_ST_DT_LY 
			FROM DIM_DATE WHERE DATE_KEY = (SELECT WEEK_ST_DATE_KEY FROM DIM_DATE WHERE TRUNC(DATE_FLD) = TRUNC((SELECT DISTINCT UPDATE_DATE FROM METRO_IT_SALES_DEPT_WTD_V2)))),
		(SELECT TO_CHAR(DATE_FLD-1, 'DD Mon YYYY') AS WTD_EN_DT, TO_CHAR(DATE_FLD_LY-1, 'DD Mon YYYY') AS WTD_EN_DT_LY 
			FROM DIM_DATE WHERE TRUNC(DATE_FLD) = TRUNC((SELECT DISTINCT UPDATE_DATE FROM METRO_IT_SALES_DEPT_WTD_V2)))
	 };
	...
	$workbook = Excel::Writer::XLSX->new("Daily Sales Performance - Summary (as of $as_of) v2.8.xlsx");
	...
	$pdf_job_1->spawn( "cmd" , q{cmd /C java ecp_FileConverter "Daily Sales Performance - Summary (as of } . $as_of . q{) v2.8.xlsx" pdf});
	$pdf_job_1->run(60);	
...
$worksheet->write($a-11, 2, "Daily Sales Performance", $bold1);
...
foreach my $i ( '2001', '2001W', '2002', '2003', '2004', '2005', '2006', '2007', '2008', '2009', '2010', '2011', '2012', '2013', '2223', '3001', '3002', '3003', '3004', '3005', '3006', '3007', '3009', '3010', '3012', '4003', '4004', '6001', '6002', '6003', '6004', '6005', '6009', '6010', '6012','6013' ){ 
...
$worksheet->write($a-11, 3, "Daily Sales Performance", $bold1);
...
$sls = $dbh->prepare (qq{
	...
	FROM
		(SELECT DISTINCT UPPER(STORE_FORMAT_DESC) FORMAT FROM METRO_IT_SALES_DEPT_V2 WHERE STORE_FORMAT <> 3)A LEFT JOIN
		...
		FROM METRO_IT_SALES_DEPT_WTD_V2
		...
		SELECT CASE WHEN STORE_FORMAT = '3' THEN MERCH_GROUP_DESC ELSE UPPER(STORE_FORMAT_DESC) END AS FORMAT, 
						...
						0 AS MTD_TARGET, 0 AS MTD_NET_TY, 0 AS MTD_NET_LY, 
						0 AS QTD_TARGET, 0 AS QTD_NET_TY, 0 AS QTD_NET_LY, 
						SUM(TARGET_SALE_VAL) YTD_TARGET, SUM(NET_SALE_OTR_TY+NET_SALE_CON_TY) YTD_NET_TY, SUM(NET_SALE_OTR_LY+NET_SALE_CON_LY) YTD_NET_LY 
						FROM METRO_IT_SALES_DEPT_V2 
		WHERE ((NEW_FLG = '$new_flg1' OR NEW_FLG = '$new_flg2') AND (MATURED_FLG = '$matured_flg1' OR MATURED_FLG = '$matured_flg2')) 
		...
		$sls2 = $dbh->prepare (qq{
			...
			FROM
			  ...
			  SUM(TARGET_SALE_VAL) AS WTD_TARGET, SUM(NET_SALE_OTR_TY+NET_SALE_CON_TY) AS WTD_NET_TY, SUM(NET_SALE_OTR_LY+NET_SALE_CON_LY) AS WTD_NET_LY, 
				  ...
				  WHERE (PER IN (SELECT DISTINCT MONTH_NAME FROM DIM_DATE_PRL WHERE YEAR = '$year' AND DATE_FLD <= TO_DATE('$mo_st_date_fld')))
					AND YEAR = (SELECT TO_CHAR(SYSDATE, 'YYYY') FROM DUAL) 
					AND MERCH_GROUP_CODE <> 'Z_OT' AND MATURED_FLG = '$flg' 
				  GROUP BY CASE WHEN STORE_FORMAT = '3' THEN MERCH_GROUP_DESC ELSE UPPER(STORE_FORMAT_DESC) END, STORE_CODE, STORE_DESCRIPTION, MATURED_FLG)
			WHERE FORMAT = '$store_format'
...
(SELECT A.FORMAT, WTD_TARGET_DS, WTD_NET_TY_DS, WTD_NET_LY_DS, MTD_TARGET_DS, MTD_NET_TY_DS, MTD_NET_LY_DS, QTD_TARGET_DS, QTD_NET_TY_DS, QTD_NET_LY_DS, YTD_TARGET_DS, YTD_NET_TY_DS, YTD_NET_LY_DS, WTD_TARGET_SU, WTD_NET_TY_SU, WTD_NET_LY_SU, MTD_TARGET_SU, MTD_NET_TY_SU, MTD_NET_LY_SU, QTD_TARGET_SU, QTD_NET_TY_SU, QTD_NET_LY_SU, YTD_TARGET_SU, YTD_NET_TY_SU, YTD_NET_LY_SU FROM
	(SELECT DISTINCT UPPER(STORE_FORMAT_DESC) FORMAT FROM METRO_IT_SALES_DEPT_V2 WHERE STORE_FORMAT <> 3)A 
	LEFT JOIN
	(	
	SELECT CASE WHEN STORE_FORMAT = '3' THEN MERCH_GROUP_DESC ELSE UPPER(STORE_FORMAT_DESC) END AS FORMAT_DS, 
							SUM(TARGET_SALE_VAL) AS WTD_TARGET_DS, SUM(NET_SALE_OTR_TY+NET_SALE_CON_TY) AS WTD_NET_TY_DS, SUM(NET_SALE_OTR_LY+NET_SALE_CON_LY) AS WTD_NET_LY_DS, 
	...
	(SELECT CASE WHEN STORE_FORMAT = '3' THEN MERCH_GROUP_DESC ELSE UPPER(STORE_FORMAT_DESC) END AS FORMAT_SU, 
							SUM(TARGET_SALE_VAL) AS WTD_TARGET_SU, SUM(NET_SALE_OTR_TY+NET_SALE_CON_TY) AS WTD_NET_TY_SU, SUM(NET_SALE_OTR_LY+NET_SALE_CON_LY) AS WTD_NET_LY_SU, 
...
SELECT A.FORMAT, WTD_TARGET_DS, WTD_NET_TY_DS, WTD_NET_LY_DS, MTD_TARGET_DS, MTD_NET_TY_DS, MTD_NET_LY_DS, QTD_TARGET_DS, QTD_NET_TY_DS, QTD_NET_LY_DS, YTD_TARGET_DS, YTD_NET_TY_DS, YTD_NET_LY_DS, WTD_TARGET_SU, WTD_NET_TY_SU, WTD_NET_LY_SU, MTD_TARGET_SU, MTD_NET_TY_SU, MTD_NET_LY_SU, QTD_TARGET_SU, QTD_NET_TY_SU, QTD_NET_LY_SU, YTD_TARGET_SU, YTD_NET_TY_SU, YTD_NET_LY_SU FROM	
	...
	(SELECT CASE WHEN STORE_FORMAT = '3' THEN MERCH_GROUP_DESC ELSE UPPER(STORE_FORMAT_DESC) END AS FORMAT_DS, 
						...
						SUM(TARGET_SALE_VAL) AS MTD_TARGET_DS, SUM(NET_SALE_OTR_TY+NET_SALE_CON_TY) AS MTD_NET_TY_DS, SUM(NET_SALE_OTR_LY+NET_SALE_CON_LY) AS MTD_NET_LY_DS,
						0 AS QTD_TARGET_DS, 0 AS QTD_NET_TY_DS, 0 AS QTD_NET_LY_DS, 
						0 AS YTD_TARGET_DS, 0 AS YTD_NET_TY_DS, 0 AS YTD_NET_LY_DS
	FROM METRO_IT_SALES_DEPT_V2 
	WHERE ((NEW_FLG = '$new_flg1' OR NEW_FLG = '$new_flg2') AND (MATURED_FLG = '$matured_flg1' OR MATURED_FLG = '$matured_flg2')) 
	...
	(SELECT CASE WHEN STORE_FORMAT = '3' THEN MERCH_GROUP_DESC ELSE UPPER(STORE_FORMAT_DESC) END AS FORMAT_SU, 
						...
						SUM(TARGET_SALE_VAL) AS MTD_TARGET_SU, SUM(NET_SALE_OTR_TY+NET_SALE_CON_TY) AS MTD_NET_TY_SU, SUM(NET_SALE_OTR_LY+NET_SALE_CON_LY) AS MTD_NET_LY_SU,
						0 AS QTD_TARGET_SU, 0 AS QTD_NET_TY_SU, 0 AS QTD_NET_LY_SU, 
						0 AS YTD_TARGET_SU, 0 AS YTD_NET_TY_SU, 0 AS YTD_NET_LY_SU
	FROM METRO_IT_SALES_DEPT_V2 
	WHERE ((NEW_FLG = '$new_flg1' OR NEW_FLG = '$new_flg2') AND (MATURED_FLG = '$matured_flg1' OR MATURED_FLG = '$matured_flg2')) 
...
SELECT A.FORMAT, WTD_TARGET_DS, WTD_NET_TY_DS, WTD_NET_LY_DS, MTD_TARGET_DS, MTD_NET_TY_DS, MTD_NET_LY_DS, QTD_TARGET_DS, QTD_NET_TY_DS, QTD_NET_LY_DS, YTD_TARGET_DS, YTD_NET_TY_DS, YTD_NET_LY_DS, WTD_TARGET_SU, WTD_NET_TY_SU, WTD_NET_LY_SU, MTD_TARGET_SU, MTD_NET_TY_SU, MTD_NET_LY_SU, QTD_TARGET_SU, QTD_NET_TY_SU, QTD_NET_LY_SU, YTD_TARGET_SU, YTD_NET_TY_SU, YTD_NET_LY_SU FROM	
	...
	(SELECT CASE WHEN STORE_FORMAT = '3' THEN MERCH_GROUP_DESC ELSE UPPER(STORE_FORMAT_DESC) END AS FORMAT_DS, 
						0 AS WTD_TARGET_DS, 0 AS WTD_NET_TY_DS, 0 AS WTD_NET_LY_DS, 
						0 AS MTD_TARGET_DS, 0 AS MTD_NET_TY_DS, 0 AS MTD_NET_LY_DS,
						SUM(TARGET_SALE_VAL) AS QTD_TARGET_DS, SUM(NET_SALE_OTR_TY+NET_SALE_CON_TY) AS QTD_NET_TY_DS, SUM(NET_SALE_OTR_LY+NET_SALE_CON_LY) AS QTD_NET_LY_DS,
						0 AS YTD_TARGET_DS, 0 AS YTD_NET_TY_DS, 0 AS YTD_NET_LY_DS
	FROM METRO_IT_SALES_DEPT_V2 
	WHERE ((NEW_FLG = '$new_flg1' OR NEW_FLG = '$new_flg2') AND (MATURED_FLG = '$matured_flg1' OR MATURED_FLG = '$matured_flg2')) 
		AND YEAR = (SELECT TO_CHAR(SYSDATE, 'YYYY') FROM DUAL) AND MERCH_GROUP_CODE = 'DS' AND MERCH_GROUP_CODE <> 'Z_OT' 
	...
	(SELECT CASE WHEN STORE_FORMAT = '3' THEN MERCH_GROUP_DESC ELSE UPPER(STORE_FORMAT_DESC) END AS FORMAT_SU, 
						0 AS WTD_TARGET_SU, 0 AS WTD_NET_TY_SU, 0 AS WTD_NET_LY_SU, 
						0 AS MTD_TARGET_SU, 0 AS MTD_NET_TY_SU, 0 AS MTD_NET_LY_SU,
						SUM(TARGET_SALE_VAL) AS QTD_TARGET_SU, SUM(NET_SALE_OTR_TY+NET_SALE_CON_TY) AS QTD_NET_TY_SU, SUM(NET_SALE_OTR_LY+NET_SALE_CON_LY) AS QTD_NET_LY_SU,
						0 AS YTD_TARGET_SU, 0 AS YTD_NET_TY_SU, 0 AS YTD_NET_LY_SU
	FROM METRO_IT_SALES_DEPT_V2 
	WHERE ((NEW_FLG = '$new_flg1' OR NEW_FLG = '$new_flg2') AND (MATURED_FLG = '$matured_flg1' OR MATURED_FLG = '$matured_flg2')) 
		AND YEAR = (SELECT TO_CHAR(SYSDATE, 'YYYY') FROM DUAL) AND MERCH_GROUP_CODE = 'SU' AND MERCH_GROUP_CODE <> 'Z_OT'
...
SELECT A.FORMAT, WTD_TARGET_DS, WTD_NET_TY_DS, WTD_NET_LY_DS, MTD_TARGET_DS, MTD_NET_TY_DS, MTD_NET_LY_DS, QTD_TARGET_DS, QTD_NET_TY_DS, QTD_NET_LY_DS, YTD_TARGET_DS, YTD_NET_TY_DS, YTD_NET_LY_DS, WTD_TARGET_SU, WTD_NET_TY_SU, WTD_NET_LY_SU, MTD_TARGET_SU, MTD_NET_TY_SU, MTD_NET_LY_SU, QTD_TARGET_SU, QTD_NET_TY_SU, QTD_NET_LY_SU, YTD_TARGET_SU, YTD_NET_TY_SU, YTD_NET_LY_SU FROM	
	...
	(SELECT CASE WHEN STORE_FORMAT = '3' THEN MERCH_GROUP_DESC ELSE UPPER(STORE_FORMAT_DESC) END AS FORMAT_DS, 
						...
						0 AS MTD_TARGET_DS, 0 AS MTD_NET_TY_DS, 0 AS MTD_NET_LY_DS, 
						0 AS QTD_TARGET_DS, 0 AS QTD_NET_TY_DS, 0 AS QTD_NET_LY_DS, 
						SUM(TARGET_SALE_VAL) AS YTD_TARGET_DS, SUM(NET_SALE_OTR_TY+NET_SALE_CON_TY) AS YTD_NET_TY_DS, SUM(NET_SALE_OTR_LY+NET_SALE_CON_LY) AS YTD_NET_LY_DS
	FROM METRO_IT_SALES_DEPT_V2 
	WHERE ((NEW_FLG = '$new_flg1' OR NEW_FLG = '$new_flg2') AND (MATURED_FLG = '$matured_flg1' OR MATURED_FLG = '$matured_flg2')) 
		AND (PER IN (SELECT DISTINCT MONTH_NAME FROM DIM_DATE_PRL WHERE YEAR = '$year' AND DATE_FLD <= TO_DATE('$mo_st_date_fld')))
		AND YEAR = (SELECT TO_CHAR(SYSDATE, 'YYYY') FROM DUAL) AND MERCH_GROUP_CODE = 'DS' AND MERCH_GROUP_CODE <> 'Z_OT' 
	...
	(SELECT CASE WHEN STORE_FORMAT = '3' THEN MERCH_GROUP_DESC ELSE UPPER(STORE_FORMAT_DESC) END AS FORMAT_SU, 
						...
						0 AS MTD_TARGET_SU, 0 AS MTD_NET_TY_SU, 0 AS MTD_NET_LY_SU, 
						0 AS QTD_TARGET_SU, 0 AS QTD_NET_TY_SU, 0 AS QTD_NET_LY_SU, 
						SUM(TARGET_SALE_VAL) AS YTD_TARGET_SU, SUM(NET_SALE_OTR_TY+NET_SALE_CON_TY) AS YTD_NET_TY_SU, SUM(NET_SALE_OTR_LY+NET_SALE_CON_LY) AS YTD_NET_LY_SU
	FROM METRO_IT_SALES_DEPT_V2 
	WHERE ((NEW_FLG = '$new_flg1' OR NEW_FLG = '$new_flg2') AND (MATURED_FLG = '$matured_flg1' OR MATURED_FLG = '$matured_flg2')) 
		AND (PER IN (SELECT DISTINCT MONTH_NAME FROM DIM_DATE_PRL WHERE YEAR = '$year' AND DATE_FLD <= TO_DATE('$mo_st_date_fld')))
		AND YEAR = (SELECT TO_CHAR(SYSDATE, 'YYYY') FROM DUAL) AND MERCH_GROUP_CODE = 'SU' AND MERCH_GROUP_CODE <> 'Z_OT' 
...
TRUNCATE TABLE METRO_IT_SALES_DEPT_WTD_V2
});
$truncate->execute();

print "Truncated table METRO_IT_SALES_DEPT_WTD_V2... \nPreparing to insert new data... \n";

$test = qq{ 

INSERT INTO METRO_IT_SALES_DEPT_WTD_V2
...
NVL((SUM((NVL(SALE_NET_VAL_OTR_TY,0))-(NVL(SALE_TOT_TAX_VAL_OTR_TY,0))-(NVL(DISC_WO_VAT_OTR_TY,0))))/1000,0) NET_SALE_OTR_TY,
SUM(SALE_TOT_QTY_OTR_TY)/1000 SALE_TOT_QTY_OTR_TY, 
NVL((SUM((NVL(SALE_NET_VAL_CON_TY,0))-(NVL(SALE_TOT_TAX_VAL_CON_TY,0))-(NVL(DISC_WO_VAT_CON_TY,0))))/1000,0) NET_SALE_CON_TY,
SUM(SALE_TOT_QTY_CON_TY)/1000 SALE_TOT_QTY_CON_TY, 
NVL((SUM((NVL(SALE_NET_VAL_OTR_LY,0))-(NVL(SALE_TOT_TAX_VAL_OTR_LY,0))-(NVL(DISC_WO_VAT_OTR_LY,0))))/1000,0) NET_SALE_OTR_LY,
SUM(SALE_TOT_QTY_OTR_LY)/1000 SALE_TOT_QTY_OTR_LY, 
NVL((SUM((NVL(SALE_NET_VAL_CON_LY,0))-(NVL(SALE_TOT_TAX_VAL_CON_LY,0))-(NVL(DISC_WO_VAT_CON_LY,0))))/1000,0) NET_SALE_CON_LY,
...
FROM (	
	SELECT TBL.PER, TBL.STORE_KEY STORE_KEY, TBL.DS_KEY DS_KEY, TBL.STORE_CODE STORE_CODE, 
		TY_OTR.SALE_NET_VAL_OTR_TY, TY_OTR.SALE_TOT_TAX_VAL_OTR_TY, TY_OTR.DISC_WO_VAT_OTR_TY, TY_OTR.SALE_TOT_QTY_OTR_TY,
		TY_CON.SALE_NET_VAL_CON_TY, TY_CON.SALE_TOT_TAX_VAL_CON_TY, TY_CON.DISC_WO_VAT_CON_TY, TY_CON.SALE_TOT_QTY_CON_TY, 
		LY_OTR.SALE_NET_VAL_OTR_LY, LY_OTR.SALE_TOT_TAX_VAL_OTR_LY, LY_OTR.DISC_WO_VAT_OTR_LY, LY_OTR.SALE_TOT_QTY_OTR_LY,
		LY_CON.SALE_NET_VAL_CON_LY, LY_CON.SALE_TOT_TAX_VAL_CON_LY, LY_CON.DISC_WO_VAT_CON_LY, LY_CON.SALE_TOT_QTY_CON_LY,
	...
	FROM
		...
		SUM(DISC_WO_VAT) DISC_WO_VAT_OTR_TY
		...
		SUM(DISC_WO_VAT) DISC_WO_VAT_CON_TY
		FROM AGG_DLY_STR_PROD AGG 
		...
		SUM(DISC_WO_VAT) DISC_WO_VAT_OTR_LY
		FROM AGG_DLY_STR_PROD AGG 
			INNER JOIN DIM_PRODUCT M ON AGG.PRODUCT_KEY = M.PRODUCT_KEY AND (M.PRODUCT_CATEGORY = 0 OR M.PRODUCT_CATEGORY IS NULL)
			INNER JOIN DIM_SUB_DEPT D ON M.MERCH_GROUP_CODE = D.MERCH_GROUP_CODE AND M.GROUP_CODE = D.GROUP_CODE AND M.GROUP2_CODE = D.GROUP2_CODE AND M.DIVISION = D.DIVISION AND M.DEPARTMENT_CODE = D.DEPARTMENT_CODE AND M.SUB_DEPARTMENT_CODE = D.SUB_DEPARTMENT_CODE
			INNER JOIN DIM_DATE DA ON AGG.DATE_KEY = DA.DATE_KEY
		...
		SUM(DISC_WO_VAT) DISC_WO_VAT_CON_LY
		FROM AGG_DLY_STR_PROD AGG 
			INNER JOIN DIM_PRODUCT M ON AGG.PRODUCT_KEY = M.PRODUCT_KEY AND M.PRODUCT_CATEGORY = 2 
			INNER JOIN DIM_SUB_DEPT D ON M.MERCH_GROUP_CODE = D.MERCH_GROUP_CODE AND M.GROUP_CODE = D.GROUP_CODE AND M.GROUP2_CODE = D.GROUP2_CODE AND M.DIVISION = D.DIVISION AND M.DEPARTMENT_CODE = D.DEPARTMENT_CODE AND M.SUB_DEPARTMENT_CODE = D.SUB_DEPARTMENT_CODE
			INNER JOIN DIM_DATE DA ON AGG.DATE_KEY = DA.DATE_KEY
	...
	SELECT TO_CHAR(DP.DATE_FLD, 'MON') PER, STORE_KEY, DS_KEY, CAST(STORE_CODE AS VARCHAR2(20)) AS STORE_CODE, 
		0 AS SALE_NET_VAL_OTR_TY, 0 AS SALE_TOT_TAX_VAL_OTR_TY, 0 AS DISC_WO_VAT_OTR_TY, 0 AS SALE_TOT_QTY_OTR_TY,
		0 AS SALE_NET_VAL_OTR_LY, 0 AS SALE_TOT_TAX_VAL_OTR_LY, 0 AS DISC_WO_VAT_OTR_LY, 0 AS SALE_TOT_QTY_CON_TY, 
		0 AS SALE_NET_VAL_CON_TY, 0 AS SALE_TOT_TAX_VAL_CON_TY, 0 AS DISC_WO_VAT_CON_LY, 0 AS SALE_TOT_QTY_OTR_LY,
		0 AS SALE_NET_VAL_CON_LY, 0 AS SALE_TOT_TAX_VAL_CON_LY, 0 AS DISC_WO_VAT_CON_LY, 0 AS SALE_TOT_QTY_CON_LY,		
...
#$bcc = 'cham.burgos@metrogaisano.com,lea.gonzaga@metrogaisano.com, annalyn.conde@metrogaisano.com';
...
$attachment_file_1 = "Daily Sales Performance - Summary (as of $as_of) v2.8.xlsx";
$attachment_file_2 = "Daily Sales Performance - Summary (as of $as_of) v2.8.pdf";
...
#$to = ' manuel.degamo@metrogaisano.com, may.sasedor@metrogaisano.com, joefrey.camu@metrogaisano.com, limuel.ulanday@metrogaisano.com, emily.silverio@metrogaisano.com, glenda.navares@metrogaisano.com, roy.igot@metrogaisano.com, luz.bitang@metrogaisano.com ';
