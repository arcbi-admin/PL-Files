...
if ($test eq 1){
	...
	$workbook = Excel::Writer::XLSX->new("Daily Sales Performance - Summary (as of $as_of) v2.2.xlsx");
	...
	$pdf_job_1->spawn( "cmd" , q{cmd /C java ecp_FileConverter "Daily Sales Performance - Summary (as of } . $as_of . q{) v2.2.xlsx" pdf});
	$pdf_job_1->run(60);	
	...
	# $workbook = Excel::Writer::XLSX->new("Daily Sales Performance (as of $as_of) v2.xlsx");
	# $bold = $workbook->add_format( bold => 1, size => 14 );
	# $bold1 = $workbook->add_format( bold => 1, size => 16 );
	# $script = $workbook->add_format( size => 8, italic => 1 );
	# $bold2 = $workbook->add_format( size => 11 );
	...
	# $headD = $workbook->add_format( border => 1, align => 'center', valign => 'center', size => 10, num_format => 9, bg_color => $abo, bold => 1 );
	# $headDPct = $workbook->add_format( border => 1, align => 'center', valign => 'right', size => 10, num_format => 9, bg_color => $abo, bold => 1 );
	...
	# $pdf_job_2->spawn( "cmd" , q{cmd /C java ecp_FileConverter "Daily Sales Performance (as of } . $as_of . q{) v2.xlsx" pdf});
	# $pdf_job_2->run(60);	
		...
		if($s1f2_row ne 0){
			...
			my $sum_LY = '=(SUM('. xl_rowcol_to_cell( $a-$counter, $col+1 ). ':' . xl_rowcol_to_cell( $a-1, $col+1 ) . '))-'. xl_rowcol_to_cell( $s1f2_row, $col+1 );
				...
				if( '=SUM('. xl_rowcol_to_cell( $a-$counter, $col+1 ). ':' . xl_rowcol_to_cell( $a-1, $col+1 ) . ')' le 0){
					...
					my $pct2sls = '=('. xl_rowcol_to_cell( $a, $col ). '/' . xl_rowcol_to_cell( $a, $col+1 ) . '-1' . ')';
...
DIM_SUB_DEPT.DIVISION,
DIM_SUB_DEPT.DEPARTMENT_CODE,
NVL((SUM(TARGET_SALE_VAL)),0) AS TARGET_SALE_VAL, 
NVL((SUM(SALE_NET_VAL))/1000,0) SALE_NET_VAL, 
NVL((SUM(SALE_TOT_TAX_VAL))/1000,0) SALE_TOT_TAX_VAL,
NVL((SUM(SALE_TOT_DISC_VAL))/1000,0) SALE_TOT_DISC_VAL, 
NVL((SUM(SALE_TOT_DISC_VAL_LY))/1000,0) SALE_TOT_DISC_VAL_LY, 
...
$attachment_file_1 = "Daily Sales Performance - Summary (as of $as_of) v2.2.xlsx";
$attachment_file_2 = "Daily Sales Performance - Summary (as of $as_of) v2.2.pdf";
