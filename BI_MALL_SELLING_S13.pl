...
if ($test eq 1){
	...
	# WHERE DATE_FLD = (SELECT TO_DATE(VALUE,'YYYY-MM-DD') FROM ADMIN_ETL_SUMMARY)
	 ...
	 $date = qq{ 	SELECT TO_CHAR(SYSDATE-6,'DD MON YYYY') DATE_FLD FROM DUAL 	 }; 
	...
	$workbook = Excel::Writer::XLSX->new("Sales by Register (S3 as of $as_of).xlsx");
...
A.Retail_Price As Retail_Price,Sum(A.Qty)As Qty,  Sum(A.Total_Retail_With_Vat) As Total_Retail_With_Vat,A.Cur_Inven As Cur_Inven  
	...
	As Total_Retail_With_Vat,  Soh.Stock_On_Hand As Cur_Inven,Soh.Av_Cost As Cost  
		...
		where trunc(SH.tran_datetime) = (SELECT TO_CHAR(SYSDATE-6,'DD MON YYYY') FROM DUAL)
			And SH.sub_tran_type in ('SALE','RETURN','LAYCOMP') And Sh.Store In (2003) And Sh.Status='P' And Its.Primary_Supp_Ind='Y'  
...
$to = 'april.agapito@metrogaisano.com, christine.lanohan@metroretail.com.ph, donna.fernando@metroretail.com.ph, elsie.lisondra@metroretail.com.ph, ester.mendoza@metroretail.com.ph, fidela.villamor@metroretail.com.ph, jay.aguilar@metroretail.com.ph, nemesio.panugan@metroretail.com.ph, rosemarie.saravia@metroretail.com.ph';
...
$subject = 'Sales by Register (S3 as of ' .$as_of. ') ';

$msgbody_file = 'message.txt';

$attachment_file = "Sales by Register (S3 as of $as_of).xlsx";
