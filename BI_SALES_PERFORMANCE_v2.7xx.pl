...
if ($test eq 1){
	...
	$workbook = Excel::Writer::XLSX->new("Daily Sales Performance - Summary (as of $as_of) v2.7xx.xlsx");
...
$sls = $dbh->prepare (qq{
	...
	FROM
		...
		WHERE ((NEW_FLG = '$new_flg1' OR NEW_FLG = '$new_flg2') AND (MATURED_FLG = '$matured_flg1' OR MATURED_FLG = '$matured_flg2')) 
			...
			AND (DIVISION = '1500' OR DIVISION = '6000') AND (PER = UPPER(TO_CHAR(TO_DATE('$mo_st_date_fld'), 'MON'))) 
		GROUP BY CASE WHEN STORE_FORMAT = '3' THEN MERCH_GROUP_DESC ELSE UPPER(STORE_FORMAT_DESC) END
		UNION ALL
		SELECT CASE WHEN STORE_FORMAT = '3' THEN MERCH_GROUP_DESC ELSE UPPER(STORE_FORMAT_DESC) END AS FORMAT, 
						0 AS WTD_TARGET, 0 AS WTD_NET_TY, 0 AS WTD_NET_LY, 
	...
	FROM
	  (SELECT CASE WHEN STORE_FORMAT = '3' THEN MERCH_GROUP_DESC ELSE UPPER(STORE_FORMAT_DESC) END AS FORMAT, STORE_CODE, STORE_DESCRIPTION, MATURED_FLG,
			  SUM(TARGET_SALE_VAL) AS WTD_TARGET, SUM(NET_SALE_OTR_TY+NET_SALE_CON_TY) AS WTD_NET_TY, SUM(NET_SALE_OTR_LY+NET_SALE_CON_LY) AS WTD_NET_LY, 
						...
						0 AS YTD_TARGET, 0 AS YTD_NET_TY, 0 AS YTD_NET_LY
	  FROM METRO_IT_SALES_DEPT_WTD
	  WHERE MERCH_GROUP_CODE <> 'Z_OT' AND (DIVISION = '1500' OR DIVISION = '6000')
	  ...
	  SELECT CASE WHEN STORE_FORMAT = '3' THEN MERCH_GROUP_DESC ELSE UPPER(STORE_FORMAT_DESC) END AS FORMAT, STORE_CODE, STORE_DESCRIPTION, MATURED_FLG,
			  ...
			  0 AS YTD_TARGET, 0 AS YTD_NET_TY, 0 AS YTD_NET_LY 
			  FROM METRO_IT_SALES_DEPT 
	  WHERE YEAR = (SELECT TO_CHAR(SYSDATE, 'YYYY') FROM DUAL) AND MERCH_GROUP_CODE <> 'Z_OT' AND (DIVISION = '1500' OR DIVISION = '6000') 
		AND (PER IN (SELECT DISTINCT MONTH_NAME FROM DIM_DATE_PRL WHERE QUARTER = '$quarter' AND YEAR = '$year' AND DATE_FLD <= TO_DATE('$mo_st_date_fld'))) 
	  GROUP BY CASE WHEN STORE_FORMAT = '3' THEN MERCH_GROUP_DESC ELSE UPPER(STORE_FORMAT_DESC) END, STORE_CODE, STORE_DESCRIPTION, MATURED_FLG	  
	  UNION ALL	  
	  SELECT CASE WHEN STORE_FORMAT = '3' THEN MERCH_GROUP_DESC ELSE UPPER(STORE_FORMAT_DESC) END AS FORMAT, STORE_CODE, STORE_DESCRIPTION, MATURED_FLG,
	  ...
	  WHERE (PER IN (SELECT DISTINCT MONTH_NAME FROM DIM_DATE_PRL WHERE YEAR = '$year' AND DATE_FLD <= TO_DATE('$mo_st_date_fld')))
		AND YEAR = (SELECT TO_CHAR(SYSDATE, 'YYYY') FROM DUAL) AND MERCH_GROUP_CODE <> 'Z_OT' AND (DIVISION = '1500' OR DIVISION = '6000') 
	  GROUP BY CASE WHEN STORE_FORMAT = '3' THEN MERCH_GROUP_DESC ELSE UPPER(STORE_FORMAT_DESC) END, STORE_CODE, STORE_DESCRIPTION, MATURED_FLG)
		...
		$sls2 = $dbh->prepare (qq{
			...
			FROM
			  ...
			  SUM(TARGET_SALE_VAL) AS WTD_TARGET, SUM(NET_SALE_OTR_TY+NET_SALE_CON_TY) AS WTD_NET_TY, SUM(NET_SALE_OTR_LY+NET_SALE_CON_LY) AS WTD_NET_LY, 
						...
						0 AS YTD_TARGET, 0 AS YTD_NET_TY, 0 AS YTD_NET_LY
				  FROM METRO_IT_SALES_DEPT_WTD
				  WHERE MERCH_GROUP_CODE <> 'Z_OT' AND MATURED_FLG = '$flg' AND (DIVISION = '1500' OR DIVISION = '6000')
				  GROUP BY CASE WHEN STORE_FORMAT = '3' THEN MERCH_GROUP_DESC ELSE UPPER(STORE_FORMAT_DESC) END, STORE_CODE, STORE_DESCRIPTION, MATURED_FLG  
				  UNION ALL	  
				  SELECT CASE WHEN STORE_FORMAT = '3' THEN MERCH_GROUP_DESC ELSE UPPER(STORE_FORMAT_DESC) END AS FORMAT, STORE_CODE, STORE_DESCRIPTION, MATURED_FLG,
						  0 AS WTD_TARGET, 0 AS WTD_NET_TY, 0 AS WTD_NET_LY, 
						  ...
						  0 AS YTD_TARGET, 0 AS YTD_NET_TY, 0 AS YTD_NET_LY 
						  FROM METRO_IT_SALES_DEPT 
				  WHERE YEAR = (SELECT TO_CHAR(SYSDATE, 'YYYY') FROM DUAL) AND MERCH_GROUP_CODE <> 'Z_OT'  AND MATURED_FLG = '$flg' AND (PER = UPPER(TO_CHAR(TO_DATE('$mo_st_date_fld'), 'MON'))) AND (DIVISION = '1500' OR DIVISION = '6000')
				  GROUP BY CASE WHEN STORE_FORMAT = '3' THEN MERCH_GROUP_DESC ELSE UPPER(STORE_FORMAT_DESC) END, STORE_CODE, STORE_DESCRIPTION, MATURED_FLG	  
				  UNION ALL	  
				  SELECT CASE WHEN STORE_FORMAT = '3' THEN MERCH_GROUP_DESC ELSE UPPER(STORE_FORMAT_DESC) END AS FORMAT, STORE_CODE, STORE_DESCRIPTION, MATURED_FLG,
						  0 AS WTD_TARGET, 0 AS WTD_NET_TY, 0 AS WTD_NET_LY, 
						  ...
						  0 AS YTD_TARGET, 0 AS YTD_NET_TY, 0 AS YTD_NET_LY 
						  FROM METRO_IT_SALES_DEPT 
				  WHERE YEAR = (SELECT TO_CHAR(SYSDATE, 'YYYY') FROM DUAL) AND MERCH_GROUP_CODE <> 'Z_OT' AND MATURED_FLG = '$flg'  AND (DIVISION = '1500' OR DIVISION = '6000')
					AND (PER IN (SELECT DISTINCT MONTH_NAME FROM DIM_DATE_PRL WHERE QUARTER = '$quarter' AND YEAR = '$year' AND DATE_FLD <= TO_DATE('$mo_st_date_fld'))) 
				  GROUP BY CASE WHEN STORE_FORMAT = '3' THEN MERCH_GROUP_DESC ELSE UPPER(STORE_FORMAT_DESC) END, STORE_CODE, STORE_DESCRIPTION, MATURED_FLG	  
				  UNION ALL	  
				  SELECT CASE WHEN STORE_FORMAT = '3' THEN MERCH_GROUP_DESC ELSE UPPER(STORE_FORMAT_DESC) END AS FORMAT, STORE_CODE, STORE_DESCRIPTION, MATURED_FLG,
						  ...
						  FROM METRO_IT_SALES_DEPT 
				  WHERE (PER IN (SELECT DISTINCT MONTH_NAME FROM DIM_DATE_PRL WHERE YEAR = '$year' AND DATE_FLD <= TO_DATE('$mo_st_date_fld')))
					AND YEAR = (SELECT TO_CHAR(SYSDATE, 'YYYY') FROM DUAL) AND MERCH_GROUP_CODE <> 'Z_OT' AND MATURED_FLG = '$flg' AND (DIVISION = '1500' OR DIVISION = '6000') 
				  GROUP BY CASE WHEN STORE_FORMAT = '3' THEN MERCH_GROUP_DESC ELSE UPPER(STORE_FORMAT_DESC) END, STORE_CODE, STORE_DESCRIPTION, MATURED_FLG)
			WHERE FORMAT = '$store_format'
...
$sls->execute();
	...
	$worksheet->write($a,25, $s->{MTD_TARGET_SU},$bodyNum);
		...
		if ($s->{MTD_NET_TY_SU} <= 0 or $s->{MTD_TARGET_SU} <= 0 ){
			...
			$worksheet->write($a,26, ($s->{MTD_NET_TY_DS}-$s->{MTD_TARGET_DS})/$s->{MTD_TARGET_DS},$bodyPct); }
			...
			$sls2 = $dbh->prepare (qq{
					...
					FROM
						(SELECT GROUP_CODE, GROUP_DESC, 
							...
							0 AS YTD_TARGET, 0 AS YTD_NET_TY, 0 AS YTD_NET_LY
						FROM METRO_IT_SALES_DEPT_WTD
						WHERE STORE_CODE = '$store' AND (DIVISION = '1500' OR DIVISION = '6000') AND MERCH_GROUP_CODE = '$merch_group_code'
						GROUP BY GROUP_CODE, GROUP_DESC
						UNION ALL				
						SELECT GROUP_CODE, GROUP_DESC, 
							0 AS WTD_TARGET, 0 AS WTD_NET_TY, 0 AS WTD_NET_LY, 
			...
			$sls3 = $dbh->prepare (qq{
				...
				FROM
					(SELECT DIVISION, DIVISION_DESC, 
							...
							0 AS YTD_TARGET, 0 AS YTD_NET_TY, 0 AS YTD_NET_LY
						FROM METRO_IT_SALES_DEPT_WTD
						WHERE STORE_CODE = '$store' AND (DIVISION = '1500' OR DIVISION = '6000') AND GROUP_CODE = '$group_code' AND MERCH_GROUP_CODE = '$merch_group_code'
						GROUP BY DIVISION, DIVISION_DESC
						UNION ALL				
						SELECT DIVISION, DIVISION_DESC, 
							0 AS WTD_TARGET, 0 AS WTD_NET_TY, 0 AS WTD_NET_LY, 
...
$attachment_file_1 = "Daily Sales Performance - Summary (as of $as_of) v2.7xx.xlsx";
