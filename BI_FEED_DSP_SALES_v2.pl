...
if ($test eq 1){
	...
	$job1->spawn( "cmd" , q{cmd /C "DSP_Jobs_V2.pl pause"});
	$job1->run(25000);
...
print "Deleting sales data from ". $upd_per1 ." to ". $upd_per2." of table METRO_IT_SALES_DEPT_V2... \nPreparing to insert new data... \n";

$test = qq{ 
DECLARE
TYPE sd_tab IS TABLE OF METRO_IT_SALES_DEPT_V2%ROWTYPE;
...
EXECUTE IMMEDIATE 'DELETE FROM METRO_IT_SALES_DEPT_V2 WHERE PER IN (''$upd_per1'', ''$upd_per2'')';

FORALL i in sales_dept_tab.first .. sales_dept_tab.last
	INSERT INTO METRO_IT_SALES_DEPT_V2 VALUES sales_dept_tab(i);
...
$test = qq{ 
DECLARE
TYPE s_tab IS TABLE OF METRO_IT_SALES_DEPT_WTD_V2%ROWTYPE;
...
SYSDATE AS UPDATE_DATE, 0 AS WEEK_NO
...
FROM
	(SELECT MONTH_IN_YEAR, TO_CHAR(DATE_FLD, 'MON' ) PER
			FROM DIM_DATE
			WHERE DATE_KEY BETWEEN $wk_st_date_key AND $wk_en_date_key + 2
	GROUP BY MONTH_IN_YEAR, TO_CHAR(DATE_FLD, 'MON' ))D,
	(SELECT DIM.STORE_KEY, DIM.STORE_FORMAT, DIM.STORE_FORMAT_DESC, DIM.STORE_CODE, DIM.STORE_DESCRIPTION, TST.NEW_FLG, TST.MATURED_FLG 
FROM DIM_STORE DIM
  LEFT JOIN (SELECT A.STORE_CODE, A.NEW_FLG, A.MATURED_FLG 
...
--EXECUTE IMMEDIATE 'DELETE FROM METRO_IT_SALES_DEPT_WTD_V2 WHERE WEEK_NO = $wk_number-4';
EXECUTE IMMEDIATE 'TRUNCATE TABLE METRO_IT_SALES_DEPT_WTD_V2';
start_time := DBMS_UTILITY.get_time;
FORALL i in sales_tab.first .. sales_tab.last
	INSERT INTO METRO_IT_SALES_DEPT_WTD_V2 VALUES sales_tab(i);
...
<html>
Hi All, <br> <br>
DSP Sales Data for Visualization and Analysis is updated, as of $as_of .<br> <br>
Period: $upd_per1 to Period: $upd_per2 , is updated .<br> <br>
