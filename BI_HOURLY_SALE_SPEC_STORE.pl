...
$test_update = qq{ SELECT CUR_TIME, UPDATE_TIME, CASE WHEN CUR_TIME <> UPDATE_TIME THEN 1 ELSE 1 END AS STATUS 
					FROM 
					( SELECT TO_CHAR(SYSDATE, 'HH12:MI:SS') CUR_TIME FROM DUAL)A,
...
$to = ' cj.jesena@metrogaisano.com, liberato.rodriguez@metrogaisano.com, emily.silverio@metrogaisano.com ';
#$to = ' kent.mamalias@metrogaisano.com, lea.gonzaga@metrogaisano.com';
...
$subject = 'Hourly Sales Performance as of ' . $cur_time;
