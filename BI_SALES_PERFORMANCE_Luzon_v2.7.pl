...
if ($test eq 1){
	...
	SELECT WTD_ST_DT, WTD_ST_DT_LY, WTD_EN_DT, WTD_EN_DT_LY FROM
		(SELECT TO_CHAR(DATE_FLD, 'DD Mon YYYY') AS WTD_ST_DT, TO_CHAR(DATE_FLD_LY, 'DD Mon YYYY') AS WTD_ST_DT_LY 
			FROM DIM_DATE WHERE DATE_KEY = (SELECT WEEK_ST_DATE_KEY FROM DIM_DATE WHERE TRUNC(DATE_FLD) = TRUNC((SELECT DISTINCT UPDATE_DATE FROM LUZON_METRO_IT_SALES_DEPT_WTD)))),
		(SELECT TO_CHAR(DATE_FLD-1, 'DD Mon YYYY') AS WTD_EN_DT, TO_CHAR(DATE_FLD_LY-1, 'DD Mon YYYY') AS WTD_EN_DT_LY 
			FROM DIM_DATE WHERE TRUNC(DATE_FLD) = TRUNC((SELECT DISTINCT UPDATE_DATE FROM LUZON_METRO_IT_SALES_DEPT_WTD)))
	 };
	...
	$workbook = Excel::Writer::XLSX->new("Daily Sales Performance- Luzon Summary (as of $as_of) v2.7.xlsx");
	...
	$pdf_job_1->spawn( "cmd" , q{cmd /C java ecp_FileConverter "Daily Sales Performance- Luzon Summary (as of } . $as_of . q{) v2.7.xlsx" pdf});
	$pdf_job_1->run(60);	
...
foreach my $i ('2004' , '2005' , '2006' , '2007' , '2009' , '2012' , '3007' , '4004' , '6002' , '2223' , '2013' , '3012' , '6012'  ){ 
...
$sls = $dbh->prepare (qq{
	...
	FROM
		(SELECT DISTINCT UPPER(STORE_FORMAT_DESC) FORMAT FROM LUZON_METRO_IT_SALES_DEPT WHERE STORE_FORMAT <> 3)A LEFT JOIN
		...
		FROM LUZON_METRO_IT_SALES_DEPT_WTD
		...
		SELECT CASE WHEN STORE_FORMAT = '3' THEN MERCH_GROUP_DESC ELSE UPPER(STORE_FORMAT_DESC) END AS FORMAT, 
						...
						0 AS QTD_TARGET, 0 AS QTD_NET_TY, 0 AS QTD_NET_LY, 
						0 AS YTD_TARGET, 0 AS YTD_NET_TY, 0 AS YTD_NET_LY 
						FROM LUZON_METRO_IT_SALES_DEPT 
...
(SELECT A.FORMAT, WTD_TARGET_DS, WTD_NET_TY_DS, WTD_NET_LY_DS, MTD_TARGET_DS, MTD_NET_TY_DS, MTD_NET_LY_DS, QTD_TARGET_DS, QTD_NET_TY_DS, QTD_NET_LY_DS, YTD_TARGET_DS, YTD_NET_TY_DS, YTD_NET_LY_DS, WTD_TARGET_SU, WTD_NET_TY_SU, WTD_NET_LY_SU, MTD_TARGET_SU, MTD_NET_TY_SU, MTD_NET_LY_SU, QTD_TARGET_SU, QTD_NET_TY_SU, QTD_NET_LY_SU, YTD_TARGET_SU, YTD_NET_TY_SU, YTD_NET_LY_SU FROM
	(SELECT DISTINCT UPPER(STORE_FORMAT_DESC) FORMAT FROM LUZON_METRO_IT_SALES_DEPT WHERE STORE_FORMAT <> 3)A 
	LEFT JOIN
	(	
...
TRUNCATE TABLE LUZON_METRO_IT_SALES_DEPT_WTD
});
$truncate->execute();

print "Truncated table LUZON_METRO_IT_SALES_DEPT_WTD... \nPreparing to insert new data... \n";

$test = qq{ 

INSERT INTO LUZON_METRO_IT_SALES_DEPT_WTD
...
FROM DIM_STORE_LUZON DIM
  LEFT JOIN (SELECT A.STORE_CODE, A.NEW_FLG, A.MATURED_FLG 
            FROM DIM_STORE_LUZON A INNER JOIN (SELECT STORE_CODE, MAX(STORE_KEY) STORE_KEY FROM DIM_STORE_LUZON GROUP BY STORE_CODE)B ON A.STORE_KEY = B.STORE_KEY AND A.STORE_CODE = B.STORE_CODE
...
DIM_STORE_LUZON.STORE_KEY, 
DIM_STORE_LUZON.STORE_FORMAT, 
CASE WHEN DIM_STORE_LUZON.STORE_CODE IN ('4002') THEN '2001W'
	ELSE DIM_STORE_LUZON.STORE_CODE END AS STORE_CODE,
...
FROM (	
	...
	GROUP BY TO_CHAR(DP.DATE_FLD, 'MON'), STORE_KEY, STORE_CODE, DS_KEY 		
		...
		) AGG_MLY_STR_DEPT_TARGET,DIM_STORE_LUZON,DIM_SUB_DEPT
WHERE DIM_STORE_LUZON.ACTIVE = 1 AND DIM_STORE_LUZON.STORE_FORMAT IN (1, 2, 3, 4, 5) AND AGG_MLY_STR_DEPT_TARGET.STORE_KEY=DIM_STORE_LUZON.STORE_KEY AND AGG_MLY_STR_DEPT_TARGET.DS_KEY=DIM_SUB_DEPT.DS_KEY
GROUP BY 
	AGG_MLY_STR_DEPT_TARGET.PER, 
	DIM_STORE_LUZON.STORE_KEY, DIM_STORE_LUZON.STORE_FORMAT, 
	CASE WHEN DIM_STORE_LUZON.STORE_CODE IN ('4002') THEN '2001W'
		ELSE DIM_STORE_LUZON.STORE_CODE END, 
...
$subject = 'Daily Sales Performance -Luzon as of ' . $as_of;

$msgbody_file = 'message_BI.txt';

$attachment_file_1 = "Daily Sales Performance- Luzon Summary (as of $as_of) v2.7.xlsx";
$attachment_file_2 = "Daily Sales Performance- Luzon Summary (as of $as_of) v2.7.pdf";
...
$subject = 'Luzon  Daily Sales Performanceas of ' . $as_of;

$msgbody_file = 'message_BI.txt';

$attachment_file_1 = "Luzon Daily Sales Performance- Summary (as of $as_of) v2.7.xlsx";
$attachment_file_2 = "Luzon Daily Sales Performance- Summary (as of $as_of) v2.7.pdf";
