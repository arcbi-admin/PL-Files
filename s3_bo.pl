...
if ($test eq 1){
	...
	$workbook = Excel::Writer::XLSX->new("Sales by Register (S6 as of 06 MAY 2015).xlsx");
...
A.Retail_Price As Retail_Price,Sum(A.Qty)As Qty,  Sum(A.Total_Retail_With_Vat) As Total_Retail_With_Vat,A.Cur_Inven As Cur_Inven  
	...
	As Total_Retail_With_Vat,  Soh.Stock_On_Hand As Cur_Inven,Soh.Av_Cost As Cost  
		...
		where trunc(SH.tran_datetime) = '06-MAY-15'
	...
	Where Till_Code In ('069','071','073','078') 
...
$subject = 'Sales by Register (S6 as of 06 MAY 2015 ) ';

$msgbody_file = 'message.txt';

$attachment_file = "Sales by Register (S6 as of 06 MAY 2015).xlsx";
