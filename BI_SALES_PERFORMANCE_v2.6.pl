...
##revised grouping for department level sheet, added OTHERS group
...
if ($test eq 1){
	...
	$workbook = Excel::Writer::XLSX->new("Daily Sales Performance - Summary (as of $as_of) v2.6.xlsx");
	...
	$pdf_job_1->spawn( "cmd" , q{cmd /C java ecp_FileConverter "Daily Sales Performance - Summary (as of } . $as_of . q{) v2.6.xlsx" pdf});
	$pdf_job_1->run(60);	
...
$total_ytd_net_ty = 0, $total_ytd_net_ly = 0 , $total_ytd_target = 0;
...
$grp_ytd_net_ty = 0, $grp_ytd_net_ly = 0 , $grp_ytd_target = 0;
$type_test = 0;
...
$worksheet->write($a-6, 2, "YTD: $yr_st_date_fld - $mo_en_date_fld vs $yr_st_date_fld_ly - $mo_en_date_fld_ly");
$worksheet->write($a-5, 2, "As of $as_of");

##========================= COMP STORES ===========================##

&heading_2;
&heading;
&query_dept($new_flg1 = 0, $new_flg2 = 0, $matured_flg1 = 1, $matured_flg2 = 1, $loc_desc = "COMP STORES", $wtd_net_ly = 0, $mtd_net_ly = 0, $qtd_net_ly = 0, $ytd_net_ly = 0);
...
&heading_2;
&heading;
&query_dept($new_flg1 = 0, $new_flg2 = 1, $matured_flg1 = 0, $matured_flg2 = 1, $loc_desc = "ALL STORES", $wtd_net_ly = 0, $mtd_net_ly = 0, $qtd_net_ly = 0, $ytd_net_ly = 0);
...
#foreach my $i ( '2001', '2001W' ){ 
	...
	&heading_2;
	&heading;
	&query_dept_store($store = $i, $wtd_net_ly = 0, $mtd_net_ly = 0, $qtd_net_ly = 0, $ytd_net_ly = 0);
...
$worksheet->write($a-6, 3, "YTD: $yr_st_date_fld - $mo_en_date_fld vs $yr_st_date_fld_ly - $mo_en_date_fld_ly");
$worksheet->write($a-5, 3, "As of $as_of");
...
$wtd_net_ly = 0, $mtd_net_ly = 0, $qtd_net_ly = 0, $ytd_net_ly = 0;
&strComp_Ds;
&strComp_Su;
&strComp_Hy;
&strComp_Nb;
...
$s1f2_ytd_net_ty = 0,	$s1f2_ytd_net_ly = 0, $s1f2_ytd_target = 0; 
...
$stor1 = '0000'; $stor2 = '0000'; $stor3 = '3009'; $stor4 = '3010'; $stor5 = '3011'; $stor6 = '3012'; $stor7 = '0000'; $stor8 = '0000'; $stor9 = '0000'; $stor10 = '0000'; $stor11 = '0000'; $stor12 = '0000';  $stor13 = '0000';   $stor14 = '0000';  $stor15 = '0000'; 
	...
	if($type_test eq 1){	&query_summary;	} 
		...
		$new_su=$a; $a+=1; $b+=1; $d=$a; $counter = 0; #ADD 1 TO VARIABLES A RESET COUNTER
			...
			foreach my $col( 7, 8, 10, 12, 13, 15, 17, 18, 20, 22, 23, 25 ){
				my $sumTY = '=SUM('.xl_rowcol_to_cell($comp_su,$col).','.xl_rowcol_to_cell($new_su,$col).')';
					...
					if ($col eq 10 or $col eq 15 or $col eq 20 or $col eq 25){
		...
		$new_su=$a; $a+=1; $b+=1; $d=$a; $counter = 0; #ADD 1 TO VARIABLES A RESET COUNTER
			...
			foreach my $col( 7, 8, 10, 12, 13, 15, 17, 18, 20, 22, 23, 25, 27, 28, 30, 32, 33, 35, 37, 38, 40, 42, 43, 45 ){
				my $sumTY = '=SUM('.xl_rowcol_to_cell($comp_su,$col).','.xl_rowcol_to_cell($new_su,$col).')';
					$worksheet->write( $a, $col, $sumTY, $bodyNum );
					
					if ($col eq 8 or $col eq 13 or $col eq 18 or $col eq 23 or $col eq 28 or $col eq 33 or $col eq 38 or $col eq 43){
						if( xl_rowcol_to_cell( $a, $col ) le 0){
					...
					elsif ($col eq 10 or $col eq 15 or $col eq 20 or $col eq 25 or $col eq 30 or $col eq 35 or $col eq 40 or $col eq 45){
						if( xl_rowcol_to_cell( $a, $col-3 ) le 0){
							$worksheet->write( $a, $col+1, '0', $bodyPct );
						}
						else{
...
$worksheet->set_column( 22, 23, 9 );
$worksheet->set_column( 24, 24, 7 );
$worksheet->set_column( 25, 25, 9 );
...
while(my $s = $sls1->fetchrow_hashref()){
	...
	while(my $s = $sls2->fetchrow_hashref()){
		...
		if($group_code ne 'JW'){
			...
			$grp_ytd_net_ty += $s->{ytd_net_ty};
			$grp_ytd_net_ly += $s->{ytd_net_ly};
			$grp_ytd_target += $s->{ytd_target};
		}
	...
	$total_ytd_net_ty += $s->{ytd_net_ty};
	$total_ytd_net_ly += $s->{ytd_net_ly};
	$total_ytd_target += $s->{ytd_target};
	...
	if ($merch_group_code eq 'DS'){
		...
		$worksheet->write($a,22, $grp_ytd_net_ty,$headNum);
		$worksheet->write($a,23, $grp_ytd_net_ly,$headNum);
		$worksheet->write($a,25, $grp_ytd_target,$headNum);
			if ($grp_ytd_net_ly <= 0){
				$worksheet->write($a,24, "",$headDPct); }
			else{
				$worksheet->write($a,24, ($grp_ytd_net_ty-$grp_ytd_net_ly)/$grp_ytd_net_ly,$headDPct); }
				
			if ($grp_ytd_net_ty <= 0 or $grp_ytd_target <= 0 ){
				$worksheet->write($a,26, "",$headPct); }
			else{
				$worksheet->write($a,26, ($grp_ytd_net_ty-$qrp_ytd_target)/$grp_ytd_target,$headDPct); }
	...
	$worksheet->write($a,22, $total_ytd_net_ty,$headNumber);
	$worksheet->write($a,23, $total_ytd_net_ly,$headNumber);
	$worksheet->write($a,25, $total_ytd_target,$headNumber);
		if ($total_ytd_net_ly <= 0){
			$worksheet->write($a,24, "",$headPct); }
		else{
			$worksheet->write($a,24, ($total_ytd_net_ty-$total_ytd_net_ly)/$total_ytd_net_ly,$headPct); }
			
		if ($total_ytd_net_ty <= 0 or $total_ytd_target <= 0 ){
			$worksheet->write($a,26, "",$headPct); }
		else{
			$worksheet->write($a,26, ($total_ytd_net_ty-$total_ytd_target)/$total_ytd_target,$headPct); }
...
while(my $s = $sls1->fetchrow_hashref()){
	...
	if ($merch_group_code eq 'DS'){
		...
		$worksheet->write($a,22, $grp_ytd_net_ty,$headNumber);
		$worksheet->write($a,23, $grp_ytd_net_ly,$headNumber);
		$worksheet->write($a,25, $grp_ytd_target,$headNumber);
			if ($grp_ytd_net_ly <= 0){
				$worksheet->write($a,24, "",$headPct); }
			else{
				$worksheet->write($a,24, ($grp_ytd_net_ty-$grp_qtd_net_ly)/$grp_ytd_net_ly,$headPct); }
				
			if ($grp_ytd_net_ty <= 0 or $grp_ytd_target <= 0 ){
				$worksheet->write($a,26, "",$headPct); }
			else{
				$worksheet->write($a,26, ($grp_ytd_net_ty-$grp_ytd_target)/$grp_ytd_target,$headPct); }
...
while(my $s = $sls->fetchrow_hashref()){
	...
	$wtd_net_ly += $s->{wtd_net_ly};
	$mtd_net_ly += $s->{mtd_net_ly};
	$qtd_net_ly += $s->{qtd_net_ly};
	$ytd_net_ly += $s->{ytd_net_ly};
...
while(my $s = $sls->fetchrow_hashref()){
	...
	if($s1f2_counter ne 2){
		...
		$worksheet->write($a,25, $s->{ytd_target},$border1);
			...
			if ($mrch1 eq 'SU' and $mrch2 eq 'SU' and ($s->{store_code} eq '2001' or $s->{store_code} eq '2001W')) {
				...
				$s1f2_ytd_net_ty += $s->{ytd_net_ty};
				$s1f2_ytd_net_ly += $s->{ytd_net_ly};
				$s1f2_ytd_target += $s->{ytd_target};
	...
	if($s1f2_counter eq 2){
		...
		$worksheet->write($a,22, $s1f2_ytd_net_ty,$border1);
		$worksheet->write($a,23, $s1f2_ytd_net_ly,$border1);
		$worksheet->write($a,25, $s1f2_ytd_target,$border1);
			if ($s1f2_ytd_net_ly <= 0){
				$worksheet->write($a,24, "",$subt); }
			else{
				$worksheet->write($a,24, ($s1f2_ytd_net_ty-$s1f2_ytd_net_ly)/$s1f2_ytd_net_ly,$subt); }
			
			if ($s1f2_ytd_net_ty <= 0 or $s1f2_ytd_target <= 0 ){
				$worksheet->write($a,26, "",$subt); }
			else{
				$worksheet->write($a,26, ($s1f2_ytd_net_ty-$s1f2_ytd_target)/$s1f2_ytd_target,$subt); }
...
$worksheet->conditional_formatting( 'H17:AP20', { type     => 'cell',  criteria => '=', value    => 0, format   => $blank });				
...
if ($mrch1 eq 'DS' and $mrch2 eq 'DS' and $mrch3 eq 'OT' ){
	...
	while(my $s = $sls->fetchrow_hashref()){
		if($s->{merch_group_code_rev} eq 'DS'){
			...
			$worksheet->write($a,37, $s->{ytd_net_ty},$border1);
			$worksheet->write($a,38, $s->{ytd_net_ly},$border1);
			$worksheet->write($a,40, $s->{ytd_target},$border1);
			
				if ($s->{ytd_net_ly} <= 0){ 				$worksheet->write($a,39, "",$subt); }
				else{ 				$worksheet->write($a,39, ($s->{ytd_net_ty}-$s->{ytd_net_ly})/$s->{ytd_net_ly},$subt); }
				
				if ($s->{ytd_net_ty} <= 0 or $s->{ytd_target} <= 0 ){ 				$worksheet->write($a,41, "",$subt); }
				else{ 				$worksheet->write($a,41, ($s->{ytd_net_ty}-$s->{ytd_target})/$s->{ytd_target},$subt); }
			
			$worksheet->write($a,42, "",$border1);
			$worksheet->write($a,43, "",$border1);
			...
			$worksheet->write($a,42, $s->{ytd_net_ty},$border1);
			$worksheet->write($a,43, $s->{ytd_net_ly},$border1);
			$worksheet->write($a,45, $s->{ytd_target},$border1);
			
				if ($s->{ytd_net_ly} <= 0){ 				$worksheet->write($a,44, "",$subt); }
				else{ 				$worksheet->write($a,44, ($s->{ytd_net_ty}-$s->{ytd_net_ly})/$s->{ytd_net_ly},$subt); }
				
				if ($s->{ytd_net_ty} <= 0 or $s->{ytd_target} <= 0 ){ 				$worksheet->write($a,46, "",$subt); }
				else{ 				$worksheet->write($a,46, ($s->{ytd_net_ty}-$s->{ytd_target})/$s->{ytd_target},$subt); }
		}
	...
	$worksheet->merge_range( $a-1, 4, $a-1, 6, $div_name3, $desc );
...
$blank = $workbook->add_format( border => 1, align => 'center', valign => 'right', size => 10, hidden => 1 );
$worksheet->conditional_formatting( 'H43:AP60', { type     => 'cell',  criteria => '=', value    => 0, format   => $blank });	
$worksheet->conditional_formatting( 'F9:AU2000',  { type => 'cell', criteria => '<', value => 0, format => $down } );			
...
if ($mrch1 eq 'DS' and $mrch2 eq 'DS' and $mrch3 eq 'OT' ){	
	...
	while(my $s = $sls->fetchrow_hashref()){
		...
		if($s1f2_counter eq 2){
			...
			$worksheet->write($a,42, $s1f2_ytd_net_ty,$border1);
			$worksheet->write($a,43, $s1f2_ytd_net_ly,$border1);
			$worksheet->write($a,45, $s1f2_ytd_target,$border1);
			
			if ($s1f2_ytd_net_ly <= 0){ 				$worksheet->write($a,44, "",$subt); }
			else{ 				$worksheet->write($a,44, ($s1f2_ytd_net_ty-$s1f2_ytd_net_ly)/$s1f2_ytd_net_ly,$subt); }
			
			if ($s1f2_ytd_net_ty <= 0 or $s1f2_ytd_target <= 0 ){ 				$worksheet->write($a,46, "",$subt); }
			else{ 				$worksheet->write($a,46, ($s1f2_ytd_net_ty-$s1f2_ytd_target)/$s1f2_ytd_target,$subt); }
...
my $csv = Text::CSV_XS->new ({ binary => 1, eol => $/ });
 open my $fh, ">", "bi_sales_perf_26.csv" or die "bi_sales_perf_26.csv: $!";
...
case when store_code in ('3010','3009','3012','6004','6009','6012') then 1 else 0 end as NEW_FLG, 
case when store_code in ('3010','3009','3012','6004','6009','6012') then 0 else 1 end as MATURED_FLG, 
SUM(TARGET_SALE_VAL) TARGET_SALE_VAL, SUM(NET_SALE_TY) NET_SALE_TY, SUM(NET_SALE_LY) NET_SALE_LY, 
SUM(MTD_TARGET_SALE_VAL) MTD_TARGET_SALE_VAL, SUM(MTD_NET_SALE_TY) MTD_NET_SALE_TY, SUM(MTD_NET_SALE_LY) MTD_NET_SALE_LY, 
SUM(QTD_TARGET_SALE_VAL) QTD_TARGET_SALE_VAL, SUM(QTD_NET_SALE_TY) QTD_NET_SALE_TY, SUM(QTD_NET_SALE_LY) QTD_NET_SALE_LY,
SUM(YTD_TARGET_SALE_VAL) YTD_TARGET_SALE_VAL, SUM(YTD_NET_SALE_TY) YTD_NET_SALE_TY, SUM(YTD_NET_SALE_LY) YTD_NET_SALE_LY
FROM (
...
, SUM(YTD.TARGET_SALE_VAL) YTD_TARGET_SALE_VAL, SUM(YTD.NET_SALE_TY) YTD_NET_SALE_TY, SUM(YTD.NET_SALE_LY) YTD_NET_SALE_LY
, SUM(NVL(WTD.TARGET_SALE_VAL,0)+NVL(WTD.NET_SALE_TY,0)+NVL(WTD.NET_SALE_LY,0)+NVL(MTD.TARGET_SALE_VAL,0)+NVL(MTD.NET_SALE_TY,0)+NVL(MTD.NET_SALE_LY,0)+NVL(QTD.TARGET_SALE_VAL,0)+NVL(QTD.NET_SALE_TY,0)+NVL(QTD.NET_SALE_LY,0)+NVL(YTD.TARGET_SALE_VAL,0)+NVL(YTD.NET_SALE_TY,0)+NVL(YTD.NET_SALE_LY,0)) SALE_CHECK
FROM
...
0 AS TARGET_SALE_VAL, 0 AS TARGET_SALE_VAL_LY, 0 AS TARGET_SALE_VAT, 0 AS TARGET_SALE_VAT_LY 	FROM
		...
		FROM AGG_DLY_STR_PROD AGG 
			INNER JOIN DIM_PRODUCT M ON AGG.PRODUCT_KEY = M.PRODUCT_KEY 
			INNER JOIN DIM_SUB_DEPT D ON M.MERCH_GROUP_CODE = D.MERCH_GROUP_CODE AND M.GROUP_CODE = D.GROUP_CODE AND M.GROUP2_CODE = D.GROUP2_CODE AND M.DIVISION = D.DIVISION AND M.DEPARTMENT_CODE = D.DEPARTMENT_CODE AND M.SUB_DEPARTMENT_CODE = D.SUB_DEPARTMENT_CODE
		WHERE DATE_KEY BETWEEN $yr_st_date_key_ly AND $mo_en_date_key_ly 
		GROUP BY STORE_KEY, DS_KEY, STORE_CODE)LY 
		ON TBL.STORE_KEY = LY.STORE_KEY AND TBL.STORE_CODE = LY.STORE_CODE AND TBL.DS_KEY = LY.DS_KEY 
	UNION ALL 
	SELECT STORE_KEY, DS_KEY, CAST(STORE_CODE AS VARCHAR2(20)) AS STORE_CODE, 
...
ON BASE.STORE_KEY = YTD.STORE_KEY AND BASE.STORE_FORMAT = YTD.STORE_FORMAT AND BASE.STORE_CODE = YTD.STORE_CODE AND BASE.MERCH_GROUP_CODE = YTD.MERCH_GROUP_CODE AND BASE.GROUP_CODE = YTD.GROUP_CODE AND BASE.DIVISION = YTD.DIVISION AND BASE.DEPARTMENT_CODE = YTD.DEPARTMENT_CODE
...
WHERE SALE_CHECK <> 0
GROUP BY 
STORE_FORMAT, STORE_FORMAT_DESC, STORE_CODE, STORE, STORE_DESCRIPTION, MERCH_GROUP_CODE_REV, MERCH_GROUP_DESC_REV, MERCH_GROUP_CODE, MERCH_GROUP_DESC, GROUP_CODE, GROUP_DESC, DIVISION, DIVISION_DESC, DEPARTMENT_CODE, DEPARTMENT_DESC, 
case when store_code in ('3010','3009','3012','6004','6009','6012') then 1 else 0 end, 
case when store_code in ('3010','3009','3012','6004','6009','6012') then 0 else 1 end
};
...
my $sth = $dbh->prepare ($test);
 ...
 close $fh or die "bi_sales_perf_26.csv: $!";
...
$to = ' chit.lazaro@metrogaisano.com, fili.mercado@metrogaisano.com, karan.malani@metrogaisano.com, julie.montano@metrogaisano.com, lia.chipeco@metrogaisano.com, marlita.portes@metrogaisano.com, jordan.mok@metrogaisano.com, jennifer.yu@metrogaisano.com, jessica.gaisano@metrogaisano.com, april.agapito@metrogaisano.com, edna.prieto@metrogaisano.com, tessie.baldezamo@metrogaisano.com, delia.jakosalem@metrogaisano.com, joyce.mirabueno@metrogaisano.com, liberato.rodriguez@metrogaisano.com, mariegrace.ong@metrogaisano.com, nerissa.lagare@metrogaisano.com, merrill.gaisano@metrogaisano.com, chedie.lim@metrogaisano.com  ';

$bcc = ' rex.cabanilla@metrogaisano.com, kent.mamalias@metrogaisano.com, lea.gonzaga@metrogaisano.com, annalyn.conde@metrogaisano.com, frank.naquines@metrogaisano.com, cham.burgos@metrogaisano.com, roel.gevana@metrogaisano.com, rlegaspi@metro.com.ph ';
...
$attachment_file_1 = "Daily Sales Performance - Summary (as of $as_of) v2.6.xlsx";
$attachment_file_2 = "Daily Sales Performance - Summary (as of $as_of) v2.6.pdf";
...
$to = ' may.sasedor@metrogaisano.com, dinah.ramirez@metrogaisano.com, jacqueline.cano@metrogaisano.com, joefrey.camu@metrogaisano.com, limuel.ulanday@metrogaisano.com, emily.silverio@metrogaisano.com, glenda.navares@metrogaisano.com, roy.igot@metrogaisano.com, luz.bitang@metrogaisano.com ';
