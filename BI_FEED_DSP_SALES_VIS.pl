...
if ($test eq 1){
	...
	$job1->spawn( "cmd" , q{cmd /C "DSP_Jobs_Vis.pl pause"});
	$job1->run(25000);
...
print "Deleting sales data from ". $upd_per1 ." to ". $upd_per2." of table METRO_IT_SALES_DEPT_VIS... \nPreparing to insert new data... \n";

$test = qq{ 
DECLARE
TYPE sd_tab IS TABLE OF METRO_IT_SALES_DEPT_VIS%ROWTYPE;
...
EXECUTE IMMEDIATE 'DELETE FROM METRO_IT_SALES_DEPT_VIS WHERE PER IN (''$upd_per1'', ''$upd_per2'')';

FORALL i in sales_dept_tab.first .. sales_dept_tab.last
	INSERT INTO METRO_IT_SALES_DEPT_VIS VALUES sales_dept_tab(i);
...
print "Deleted old records from table METRO_IT_SALES_DEPT_WTD_VIS... \nPreparing to insert new data... \n";

$test = qq{ 
DECLARE
TYPE s_tab IS TABLE OF METRO_IT_SALES_DEPT_WTD_VIS%ROWTYPE;
...
--EXECUTE IMMEDIATE 'DELETE FROM METRO_IT_SALES_DEPT_WTD_VIS WHERE WEEK_NO = $wk_number-4';
EXECUTE IMMEDIATE 'TRUNCATE TABLE METRO_IT_SALES_DEPT_WTD_VIS';
start_time := DBMS_UTILITY.get_time;
FORALL i in sales_tab.first .. sales_tab.last
	INSERT INTO METRO_IT_SALES_DEPT_WTD_VIS VALUES sales_tab(i);
...
print "Done Deleting current periods records on table METRO_IT_INSTOCK_DEPT_VIS... \nPreparing to Insert... \n";

$test = '
DECLARE
TYPE inst_tab IS TABLE OF METRO_IT_INSTOCK_DEPT_VIS%ROWTYPE;
		...
		FROM
			(SELECT A.STORE_FORMAT, F.FORMAT_NAME STORE_FORMAT_DESC, STORE, STORE_NAME, NEW_FLG, MATURED_FLG, REGION FROM 
				...
				LEFT JOIN (SELECT A.STORE_CODE, A.NEW_FLG, A.MATURED_FLG, A.REGION
						FROM DIM_STORE_VIS A 
							INNER JOIN (SELECT STORE_CODE, MAX(STORE_KEY) STORE_KEY FROM DIM_STORE_VIS GROUP BY STORE_CODE)B ON A.STORE_KEY = B.STORE_KEY AND A.STORE_CODE = B.STORE_CODE
...
EXECUTE IMMEDIATE \'TRUNCATE TABLE METRO_IT_INSTOCK_DEPT_VIS\';

FORALL i in instock_tab.first .. instock_tab.last
	INSERT INTO METRO_IT_INSTOCK_DEPT_VIS VALUES instock_tab(i);
