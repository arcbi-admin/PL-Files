...
if ($test eq 1){
	...
	while (my $x = $sth_date_1->fetchrow_hashref()) {
		...
		$day_st_date_key_ly = 804;
		$day_st_date_fld = $x->{DATE_FLD};
		#$day_st_date_fld_ly = $x->{DATE_FLD_LY};
		$day_st_date_fld_ly = '15 Mar 2015';
		$as_of = $x->{DATE_FLD};
	}
	
	$event_st_date_key = 1173; #18-Mar-16: Sidewalk Sale 2016
	$event_en_date_key = 1175; #20-Mar-16: Sidewalk Sale 2016
	...
	$workbook = Excel::Writer::XLSX->new("Event Sales Performance - Summary (as of $as_of) v2.3.xlsx");
...
$worksheet->write($a-9, 2, "Event Sales Performance (Sidewalk Sale 2016)", $bold1);
...
foreach my $i ( '2001', '2001W', '2002', '2003', '2004', '2005', '2006', '2007', '2008', '2009', '2010', '2011', '2012', '2013', '2223', '3001', '3002', '3003', '3004', '3005', '3006', '3007', '3009', '3010', '3012', '4003', '4004', '6001', '6002', '6003', '6004', '6005','6006', '6009', '6010', '6012','6013','6011' ){ 
...
$worksheet->write($a-9, 3, "Event Sales Performance (Sidewalk Sale 2016)", $bold1);
...
$stor1 = '0000'; $stor2 = '0000'; $stor3 = '3009'; $stor4 = '3010'; $stor5 = '3011'; $stor6 = '3012'; $stor7 = '0000'; $stor8 = '0000'; $stor9 = '0000'; $stor10 = '0000'; $stor11 = '0000'; $stor12 = '0000';  $stor13 = '0000';   $stor14 = '0000'; 
	...
	if($type_test eq 1){	&query_summary;	} 
		...
		$new_su=$a; $a+=1; $b+=1; $d=$a; $counter = 0; #ADD 1 TO VARIABLES A RESET COUNTER
			...
			foreach my $col( 7, 8, 10, 12, 13, 15, 17, 18, 20, 22, 23, 25 ){
				my $sumTY = '=SUM('.xl_rowcol_to_cell($comp_su,$col).','.xl_rowcol_to_cell($new_su,$col).')';
					$worksheet->write( $a, $col, $sumTY, $bodyNum );
					
				if ($col eq 8 or $col eq 13 or $col eq 18 or $col eq 23){
					my $pct2sls = '=('. xl_rowcol_to_cell( $a, $col-1 ). '/' . xl_rowcol_to_cell( $a, $col ) . '-1' . ')';
					$worksheet->write( $a, $col+1, $pct2sls, $bodyPct ); 				 			}
				elsif ($col eq 10 or $col eq 15 or $col eq 20 or $col eq 25){
					my $pct2sls = '=('. xl_rowcol_to_cell( $a, $col-3 ). '/' . xl_rowcol_to_cell( $a, $col ) . '-1' . ')';
					$worksheet->write( $a, $col+1, $pct2sls, $bodyPct ); }
			}
...
$worksheet->merge_range( $a-2, 7, $a-2, 11, 'Previous Day', $subhead );
...
$worksheet->write($a-3, 3, "in 000's", $script);
$worksheet->merge_range( $a-2, 7, $a-2, 16, 'Previous Day', $subhead );
...
while(my $s = $sls1->fetchrow_hashref()){
		...
		while(my $s = $sls3->fetchrow_hashref()){
			...
			while(my $s = $sls4->fetchrow_hashref()){
				...
				if ($s->{mtd_net_ty} <= 0 or $s->{mtd_target} <= 0 ){
					...
					$worksheet->write($a,16, ($s->{mtd_net_ty})/$s->{mtd_target},$subt); }
			...
			$worksheet->write($a,15, $s->{mtd_target},$bodyNum);
				...
				if ($s->{mtd_net_ty} <= 0 or $s->{mtd_target} <= 0 ){
					...
					$worksheet->write($a,16, ($s->{mtd_net_ty})/$s->{mtd_target},$bodyPct); }
	...
	if ($merch_group_code eq 'DS'){
		...
		$worksheet->write($a,15, $grp_mtd_target,$headNum);
			...
			if ($grp_mtd_net_ty <= 0 or $grp_mtd_target <= 0 ){
				...
				$worksheet->write($a,16, ($grp_mtd_net_ty)/$grp_mtd_target,$headDPct); }
	...
	$worksheet->write($a,15, $s->{mtd_target},$headNumber);
		...
		if ($s->{mtd_net_ty} <= 0 or $s->{mtd_target} <= 0 ){
			...
			$worksheet->write($a,16, ($s->{mtd_net_ty})/$s->{mtd_target},$headPct); }
	...
	$worksheet->write($a,15, $total_mtd_target,$headNumber);
		...
		if ($total_mtd_net_ty <= 0 or $total_mtd_target <= 0 ){
			...
			$worksheet->write($a,16, ($total_mtd_net_ty)/$total_mtd_target,$headPct); }
	...
	if ($merch_group_code eq 'DS'){
		...
		$worksheet->write($a,15, $grp_mtd_target,$headNumber);
			...
			if ($grp_mtd_net_ty <= 0 or $grp_mtd_target <= 0 ){
				...
				$worksheet->write($a,16, ($grp_mtd_net_ty)/$grp_mtd_target,$headPct); }
...
while(my $s = $sls->fetchrow_hashref()){
	...
	if($s1f2_counter eq 2){
		...
		if ($s1f2_mtd_net_ty <= 0 or $s1f2_mtd_target <= 0 ){
			...
			$worksheet->write($a,16, ($s1f2_mtd_net_ty)/$s1f2_mtd_target,$subt); }
...
if ($mrch1 eq 'DS' and $mrch2 eq 'DS' and $mrch3 eq 'OT' ){
	...
	while(my $s = $sls->fetchrow_hashref()){
		...
		else{ 		$worksheet->write($a,21, ($s->{mtd_net_ty})/$s->{mtd_target},$subt); 	}
	...
	while(my $s = $sls->fetchrow_hashref()){
		...
		else{ 		$worksheet->write($a,26, ($s->{mtd_net_ty})/$s->{mtd_target},$subt); 	}
...
if ($mrch1 eq 'DS' and $mrch2 eq 'DS' and $mrch3 eq 'OT' ){	
	...
	while(my $s = $sls->fetchrow_hashref()){
		...
		if($s1f2_counter eq 2){
			...
			else{ 				$worksheet->write($a,26, ($s1f2_mtd_net_ty)/$s1f2_mtd_target,$subt); }
...
case when store_code in ('3010','3009','3012','6004','6009','6012','6013','6011','6006') then 1 else 0 end as NEW_FLG, 
case when store_code in ('3010','3009','3012','6004','6009','6012','6013','6011','6006') then 0 else 1 end as MATURED_FLG, 
...
$subject = 'Resending:Event Sales Performance (Sidewalk Sale 2016) as of ' . $as_of;

$msgbody_file = 'message_event.txt';

$attachment_file_1 = "Event Sales Performance - Summary (as of $as_of) v2.3.xlsx";
...
$subject = 'Event Sales Performance (Sidewalk Sale 2016) as of ' . $as_of;
