...
if ($test eq 1){
	...
	SELECT WEEK_NUMBER_THIS_YEAR, DATE_KEY, TO_CHAR(DATE_FLD, 'DD Mon YYYY') DATE_FLD, WEEK_ST_DATE_KEY, WEEK_END_DATE_KEY 
	FROM DIM_DATE
	WHERE DATE_FLD = (SELECT AGG_DLY_END_DATE_FLD FROM ADMIN_ETL_DATE_PARAMETER)-3
	 };
	...
	$workbook = Excel::Writer::XLSX->new("Monthly Sales Performance - Summary (as of $as_of) v2.4.xlsx");
...
$worksheet->write($a-9, 3, "WTD: $wk_st_date_fld - $wk_en_date_fld vs $wk_st_date_fld_ly - $wk_en_date_fld_ly");
...
$worksheet->merge_range( $a-2, 7, $a-2, 11, 'MONTH', $subhead );
...
$worksheet->write($a-3, 3, "in 000's", $script);
$worksheet->merge_range( $a-2, 7, $a-2, 16, 'MONTH', $subhead );
...
my $csv = Text::CSV_XS->new ({ binary => 1, eol => $/ });
 open my $fh, ">", "bi_sales_perf_mtd.csv" or die "bi_sales_perf_mtd.csv: $!";
...
0 AS TARGET_SALE_VAL, 0 AS TARGET_SALE_VAL_LY, 0 AS TARGET_SALE_VAT, 0 AS TARGET_SALE_VAT_LY 	FROM
		...
		WHERE DATE_KEY BETWEEN 344 AND 365 
	...
	WHERE A.DATE_KEY BETWEEN 344 AND 365 
...
ON BASE.STORE_FORMAT = QTD.STORE_FORMAT AND BASE.STORE_CODE = QTD.STORE_CODE AND BASE.MERCH_GROUP_CODE = QTD.MERCH_GROUP_CODE AND BASE.GROUP_CODE = QTD.GROUP_CODE AND BASE.DIVISION = QTD.DIVISION AND BASE.DEPARTMENT_CODE = QTD.DEPARTMENT_CODE
GROUP BY
BASE.STORE_FORMAT, 
BASE.STORE_FORMAT_DESC, 
BASE.STORE_CODE, 
...
my $sth = $dbh->prepare ($test);
 ...
 close $fh or die "bi_sales_perf_mtd.csv: $!";
...
$subject = 'Monthly Sales Performance as of ' . $as_of;
# $msgbody_file = 'message_BI.txt';
$msgbody_file = 'message_BI_YTD.txt';

$attachment_file_1 = "Monthly Sales Performance - Summary (as of $as_of) v2.4.xlsx";
