...
if ($test eq 1){
	...
	SELECT WTD_ST_DT, WTD_ST_DT_LY, WTD_EN_DT, WTD_EN_DT_LY FROM
		(SELECT TO_CHAR(DATE_FLD, 'DD Mon YYYY') AS WTD_ST_DT, TO_CHAR(DATE_FLD_LY, 'DD Mon YYYY') AS WTD_ST_DT_LY 
			FROM DIM_DATE WHERE DATE_KEY = (SELECT WEEK_ST_DATE_KEY FROM DIM_DATE WHERE TRUNC(DATE_FLD) = TRUNC((SELECT DISTINCT UPDATE_DATE FROM METRO_IT_SALES_DEPT_WTD_V2)-1))),
	...
	$workbook = Excel::Writer::XLSX->new("Daily Sales Performance - Summary (as of $as_of) v1.8.xlsx");
	...
	$pdf_job_1->spawn( "cmd" , q{cmd /C java ecp_FileConverter "Daily Sales Performance - Summary (as of } . $as_of . q{) v1.8.xlsx" pdf});
	$pdf_job_1->run(60);	
...
print "Deleted old records from table METRO_IT_SALES_DEPT_WTD_V2... \nPreparing to insert new data... \n";

$test = qq{ 

INSERT /*+ APPEND */ INTO METRO_IT_SALES_DEPT_WTD_V2
...
FROM (	
	...
	FROM
		...
		SUM(DISC_WO_VAT) SALE_TOT_DISC_VAL_OTR_TY
		...
		SUM(DISC_WO_VAT) SALE_TOT_DISC_VAL_CON_TY
		FROM AGG_DLY_STR_PROD AGG 
		...
		SUM(DISC_WO_VAT) SALE_TOT_DISC_VAL_OTR_LY
		FROM AGG_DLY_STR_PROD AGG 
			INNER JOIN DIM_PRODUCT M ON AGG.PRODUCT_KEY = M.PRODUCT_KEY AND (M.PRODUCT_CATEGORY = 0 OR M.PRODUCT_CATEGORY IS NULL)
			INNER JOIN DIM_SUB_DEPT D ON M.MERCH_GROUP_CODE = D.MERCH_GROUP_CODE AND M.GROUP_CODE = D.GROUP_CODE AND M.GROUP2_CODE = D.GROUP2_CODE AND M.DIVISION = D.DIVISION AND M.DEPARTMENT_CODE = D.DEPARTMENT_CODE AND M.SUB_DEPARTMENT_CODE = D.SUB_DEPARTMENT_CODE
			INNER JOIN DIM_DATE DA ON AGG.DATE_KEY = DA.DATE_KEY
		...
		SUM(DISC_WO_VAT) SALE_TOT_DISC_VAL_CON_LY
		FROM AGG_DLY_STR_PROD AGG 
			INNER JOIN DIM_PRODUCT M ON AGG.PRODUCT_KEY = M.PRODUCT_KEY AND M.PRODUCT_CATEGORY = 2 
			INNER JOIN DIM_SUB_DEPT D ON M.MERCH_GROUP_CODE = D.MERCH_GROUP_CODE AND M.GROUP_CODE = D.GROUP_CODE AND M.GROUP2_CODE = D.GROUP2_CODE AND M.DIVISION = D.DIVISION AND M.DEPARTMENT_CODE = D.DEPARTMENT_CODE AND M.SUB_DEPARTMENT_CODE = D.SUB_DEPARTMENT_CODE
			INNER JOIN DIM_DATE DA ON AGG.DATE_KEY = DA.DATE_KEY
...
$attachment_file_1 = "Daily Sales Performance - Summary (as of $as_of) v1.8.xlsx";
$attachment_file_2 = "Daily Sales Performance - Summary (as of $as_of) v1.8.pdf";
