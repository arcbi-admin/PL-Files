...
if ($test eq 1){
	...
	FROM DIM_DATE
	WHERE DATE_FLD = (SELECT AGG_DLY_END_DATE_FLD FROM ADMIN_ETL_DATE_PARAMETER)-385
	 };
	...
	#$job1->spawn( "cmd" , q{cmd /C "COMPILED_MARGIN_SALES_PERFORMANCE_v1.6_YTD.pl pause"});
	#$job1->run(25000);
...
DELETE FROM METRO_IT_MARGIN_DEPT_2014 WHERE PER = SUBSTR('$period', 1, 3)
});
$truncate->execute();

$truncate->finish();

print "Done Deleting current periods records on table METRO_IT_MARGIN_DEPT_2014... \nPreparing to Insert... \n";

$test = '
INSERT INTO METRO_IT_MARGIN_DEPT_2014 
...
FROM
	...
	(SELECT T.PER, T.LOCATION, T.GROUP_NO, SUM(T.UNITS) OTR_UNITS, SUM(T.OTR_COST) OTR_COST, SUM(T.OTR_RETAIL) OTR_RETAIL, SUM((NVL(T.OTR_RETAIL,0))-(NVL(T.TOTAL_COST,0))) OTR_MARGIN FROM
		(SELECT TO_CHAR(TDH.TRAN_DATE, \'MON\') PER, TDH.LOCATION, DEPS.GROUP_NO, TDH.UNITS UNITS, TDH.TOTAL_COST OTR_COST, TDH.TOTAL_RETAIL OTR_RETAIL, 
			...
			FROM RMSPRD.TRAN_DATA_HISTORY_24122014@RMS TDH 
