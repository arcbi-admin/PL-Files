...
if ($test eq 1){
	...
	$workbook = Excel::Writer::XLSX->new("Daily Sales Performance - Summary (as of $as_of) v2.4.xlsx");
...
my $csv = Text::CSV_XS->new ({ binary => 1, eol => $/ });
 open my $fh, ">", "bi_sales_perf_24.csv" or die "bi_sales_perf_24.csv: $!";
...
my $sth = $dbh->prepare ($test);
 ...
 close $fh or die "bi_sales_perf_24.csv: $!";
