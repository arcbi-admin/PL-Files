...
if ($test eq 1){
	...
	$workbook = Excel::Writer::XLSX->new("test bi+rms Daily Sales Performance - Summary (as of $as_of) v1.3.xlsx");
...
my $csv = Text::CSV_XS->new ({ binary => 1, eol => $/ });
 open my $fh, ">", "bi_sales_perf_1.csv" or die "bi_sales_perf_1.csv: $!";
...
0 AS TARGET_SALE_VAL, 0 AS TARGET_SALE_VAL_LY, 0 AS TARGET_SALE_VAT, 0 AS TARGET_SALE_VAT_LY 	FROM
		...
		SUM(SALE_TOT_DISC_VAL) SALE_TOT_DISC_VAL FROM
				(SELECT STORE_KEY,DS_KEY,CAST(STORE_CODE AS VARCHAR2(20)) AS STORE_CODE,
				SUM(SALE_NET_VAL) AS SALE_NET_VAL,
				SUM(SALE_TOT_TAX_VAL) SALE_TOT_TAX_VAL,
				...
				WHERE DATE_KEY BETWEEN $wk_st_date_key AND $wk_en_date_key-1
				GROUP BY STORE_KEY, DS_KEY, STORE_CODE
				UNION ALL
				SELECT ST.STORE_KEY, DS.DS_KEY, CAST(RMS.STORE_CODE AS VARCHAR2(20)) AS STORE_CODE, 
				RMS.SALE_AMT_NO_DEDUCTIONS SALE_NET_VAL, 
				TAX_AMT SALE_TOT_TAX_VAL, 
				DISC_AMT SALE_TOT_DISC_VAL
				FROM METRO_IT_RMS_DAILY_SALE2 RMS
					JOIN DIM_SUB_DEPT DS ON RMS.SUBDEPARTMENT_CODE = DS.SUB_DEPARTMENT_CODE AND RMS.DEPARTMENT_CODE = DS.DEPARTMENT_CODE
					JOIN DIM_STORE ST ON RMS.STORE_CODE = ST.STORE_CODE AND ST.LATEST = 1)
...
0 AS TARGET_SALE_VAL, 0 AS TARGET_SALE_VAL_LY, 0 AS TARGET_SALE_VAT, 0 AS TARGET_SALE_VAT_LY 	FROM
		...
		SUM(SALE_TOT_DISC_VAL) SALE_TOT_DISC_VAL FROM
				...
				FROM AGG_DLY_STR_PROD AGG
					...
					JOIN DIM_SUB_DEPT D ON M.MERCH_GROUP_CODE = D.MERCH_GROUP_CODE AND M.GROUP_CODE = D.GROUP_CODE AND M.GROUP2_CODE = D.GROUP2_CODE 
						AND M.DIVISION = D.DIVISION AND M.DEPARTMENT_CODE = D.DEPARTMENT_CODE AND M.SUB_DEPARTMENT_CODE = D.SUB_DEPARTMENT_CODE
				WHERE DATE_KEY BETWEEN $mo_st_date_key AND $mo_en_date_key-1
				GROUP BY STORE_KEY, DS_KEY, STORE_CODE
				UNION ALL
				SELECT ST.STORE_KEY, DS.DS_KEY, CAST(RMS.STORE_CODE AS VARCHAR2(20)) AS STORE_CODE, 
				RMS.SALE_AMT_NO_DEDUCTIONS SALE_NET_VAL, 
...
0 AS TARGET_SALE_VAL, 0 AS TARGET_SALE_VAL_LY, 0 AS TARGET_SALE_VAT, 0 AS TARGET_SALE_VAT_LY 	FROM
		...
		SUM(SALE_TOT_DISC_VAL) SALE_TOT_DISC_VAL FROM
				...
				FROM AGG_DLY_STR_PROD AGG
					...
					JOIN DIM_SUB_DEPT D ON M.MERCH_GROUP_CODE = D.MERCH_GROUP_CODE AND M.GROUP_CODE = D.GROUP_CODE AND M.GROUP2_CODE = D.GROUP2_CODE 
						AND M.DIVISION = D.DIVISION AND M.DEPARTMENT_CODE = D.DEPARTMENT_CODE AND M.SUB_DEPARTMENT_CODE = D.SUB_DEPARTMENT_CODE
				WHERE DATE_KEY BETWEEN $qu_st_date_key AND $qu_en_date_key-1
				GROUP BY STORE_KEY, DS_KEY, STORE_CODE
				UNION ALL
				SELECT ST.STORE_KEY, DS.DS_KEY, CAST(RMS.STORE_CODE AS VARCHAR2(20)) AS STORE_CODE, 
				RMS.SALE_AMT_NO_DEDUCTIONS SALE_NET_VAL, 
...
my $sth = $dbh->prepare ($test);
 ...
 close $fh or die "bi_sales_perf_1.csv: $!";
