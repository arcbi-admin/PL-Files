...
if ($test eq 1){
	...
	$workbook = Excel::Writer::XLSX->new("Sales by Register (S9 as of $as_of).xlsx");
...
A.Retail_Price As Retail_Price,Sum(A.Qty)As Qty,  Sum(A.Total_Retail_With_Vat) As Total_Retail_With_Vat,A.Cur_Inven As Cur_Inven  
	...
	As Total_Retail_With_Vat,  Soh.Stock_On_Hand As Cur_Inven,Soh.Av_Cost As Cost  
		...
		where trunc(SH.tran_datetime) = (SELECT TO_CHAR(SYSDATE-1,'DD MON YYYY') FROM DUAL)
			And SH.sub_tran_type in ('SALE','RETURN','LAYCOMP') And Sh.Store In (2009) And Sh.Status='P' And Its.Primary_Supp_Ind='Y'  
...
#$to = 'april.agapito@metrogaisano.com, christine.lanohan@metroretail.com.ph, cristina.de_asis@metroretail.com.ph, nemesio.panugan@metroretail.com.ph, rhoda.camporedondo@metroretail.com.ph, s9dept.supervisors@metroretail.com.ph';
$to='april.agapito@metroretail.com.ph,cristina.de_asis@metroretail.com.ph,nemesio.panugan@metroretail.com.ph,christine.lanohan@metroretail.com.ph,ester.mendoza@metroretail.com.ph,rosemarie.saravia@metroretail.com.ph,s9dept.supervisors@metroretail.com.ph, rhoda.camporedondo@metroretail.com.ph,jay.aguilar@metroretail.com.ph';

$bcc = 'rex.cabanilla@metrogaisano.com, lea.gonzaga@metrogaisano.com';

$from = 'Report Mailer<report.mailer@metrogaisano.com>';
		
$subject = 'Sales by Register (S9 as of ' .$as_of. ') ';

$msgbody_file = 'message.txt';

$attachment_file = "Sales by Register (S9 as of $as_of).xlsx";
