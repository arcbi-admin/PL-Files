...
if ($test eq 1){
	...
	$workbook = Excel::Writer::XLSX->new("YTD Sales Performance - Summary (as of $as_of) v2.4.xlsx");
	...
	# $pdf_job_1->spawn( "cmd" , q{cmd /C java ecp_FileConverter "Daily Sales Performance - Summary (as of } . $as_of . q{) v2.4.xlsx" pdf});
	# $pdf_job_1->run(60);	
...
$worksheet->write($a-9, 3, "YTD: 01 Jan 2014 - 31 Dec 2014 vs  - 01 Jan 2014");
...
$worksheet->merge_range( $a-2, 7, $a-2, 11, 'YTD', $subhead );
...
$worksheet->write($a-3, 3, "in 000's", $script);
$worksheet->merge_range( $a-2, 7, $a-2, 16, 'YTD', $subhead );
...
my $csv = Text::CSV_XS->new ({ binary => 1, eol => $/ });
 open my $fh, ">", "bi_sales_perf_ytd.csv" or die "bi_sales_perf_ytd.csv: $!";
...
0 AS TARGET_SALE_VAL, 0 AS TARGET_SALE_VAL_LY, 0 AS TARGET_SALE_VAT, 0 AS TARGET_SALE_VAT_LY 	FROM
		...
		WHERE DATE_KEY BETWEEN 366 AND 730 
		...
		FROM AGG_DLY_STR_PROD AGG 
			INNER JOIN DIM_PRODUCT M ON AGG.PRODUCT_KEY = M.PRODUCT_KEY 
			INNER JOIN DIM_SUB_DEPT D ON M.MERCH_GROUP_CODE = D.MERCH_GROUP_CODE AND M.GROUP_CODE = D.GROUP_CODE AND M.GROUP2_CODE = D.GROUP2_CODE AND M.DIVISION = D.DIVISION AND M.DEPARTMENT_CODE = D.DEPARTMENT_CODE AND M.SUB_DEPARTMENT_CODE = D.SUB_DEPARTMENT_CODE
		WHERE DATE_KEY BETWEEN 1 AND 365 
	...
	WHERE A.DATE_KEY BETWEEN 366 AND 730 
...
my $sth = $dbh->prepare ($test);
 ...
 close $fh or die "bi_sales_perf_ytd.csv: $!";
...
$bcc = ' rex.cabanilla@metrogaisano.com, kent.mamalias@metrogaisano.com, lea.gonzaga@metrogaisano.com, christopher.calalang@metrogaisano.com, annalyn.conde@metrogaisano.com, fnaquines@metro.com.ph, cham.burgos@metrogaisano.com, roel.gevana@metrogaisano.com, rlegaspi@metro.com.ph ';
...
$attachment_file_1 = "Daily Sales Performance - Summary (as of $as_of) v2.4.xlsx";
$attachment_file_2 = "Daily Sales Performance - Summary (as of $as_of) v2.4.pdf";
