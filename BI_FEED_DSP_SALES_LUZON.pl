...
if ($test eq 1){
	...
	$job1->spawn( "cmd" , q{cmd /C "DSP_Jobs_Luzon.pl pause"});
	$job1->run(25000);
...
print "Deleting sales data from ". $upd_per1 ." to ". $upd_per2." of table LUZON_METRO_IT_SALES_DEPT... \nPreparing to insert new data... \n";

$test = qq{ 
DECLARE
TYPE sd_tab IS TABLE OF LUZON_METRO_IT_SALES_DEPT%ROWTYPE;
...
EXECUTE IMMEDIATE 'DELETE FROM LUZON_METRO_IT_SALES_DEPT WHERE PER IN (''$upd_per1'', ''$upd_per2'')';

FORALL i in sales_dept_tab.first .. sales_dept_tab.last
	INSERT INTO LUZON_METRO_IT_SALES_DEPT VALUES sales_dept_tab(i);
...
$test = qq{ 
DECLARE
TYPE s_tab IS TABLE OF LUZON_METRO_IT_SALES_DEPT_WTD%ROWTYPE;
...
--EXECUTE IMMEDIATE 'DELETE FROM LUZON_METRO_IT_SALES_DEPT_WTD WHERE WEEK_NO = $wk_number-4';
EXECUTE IMMEDIATE 'TRUNCATE TABLE LUZON_METRO_IT_SALES_DEPT_WTD';
start_time := DBMS_UTILITY.get_time;
FORALL i in sales_tab.first .. sales_tab.last
	INSERT INTO LUZON_METRO_IT_SALES_DEPT_WTD VALUES sales_tab(i);
...
print "Done Deleting current periods records on table LUZON_METRO_IT_INSTOCK_DEPT... \nPreparing to Insert... \n";

$test = '
DECLARE
TYPE inst_tab IS TABLE OF LUZON_METRO_IT_INSTOCK_DEPT%ROWTYPE;
		...
		FROM
			(SELECT A.STORE_FORMAT, F.FORMAT_NAME STORE_FORMAT_DESC, STORE, STORE_NAME, NEW_FLG, MATURED_FLG, REGION FROM 
				...
				LEFT JOIN (SELECT A.STORE_CODE, A.NEW_FLG, A.MATURED_FLG, A.REGION
						FROM DIM_STORE_LUZON A 
							INNER JOIN (SELECT STORE_CODE, MAX(STORE_KEY) STORE_KEY FROM DIM_STORE_LUZON GROUP BY STORE_CODE)B ON A.STORE_KEY = B.STORE_KEY AND A.STORE_CODE = B.STORE_CODE
...
EXECUTE IMMEDIATE \'TRUNCATE TABLE LUZON_METRO_IT_INSTOCK_DEPT\';

FORALL i in instock_tab.first .. instock_tab.last
	INSERT INTO LUZON_METRO_IT_INSTOCK_DEPT VALUES instock_tab(i);
...
#$cc = 'kent.mamalias@metrogaisano.com, lea.gonzaga@metrogaisano.com, cham.burgos@metrogaisano.com';
